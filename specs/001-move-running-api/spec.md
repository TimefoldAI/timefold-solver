...
- **FR-001c**: System MUST expose a builder pattern API using method chaining (e.g., `MoveRunner.on(solution).execute(move)` for permanent execution, `MoveRunner.on(solution).executeTemporarily(move, assertions -> { ... })` for temporary execution)
