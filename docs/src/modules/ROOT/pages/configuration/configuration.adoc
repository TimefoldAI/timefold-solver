...
Solving a planning problem with Timefold consists of the following steps:
...
*Timefold makes it relatively easy to switch optimization algorithm(s) just by changing the configuration.* There is even a xref:benchmarking-and-tweaking/benchmarking-and-tweaking.adoc#benchmarker[Benchmarker] which allows you to play out different configurations against each other and report the most appropriate configuration for your use case.
...
Timefold needs to be told which classes in your domain model are planning entities, which properties are planning variables, etc.
...
Timefold by default accesses your domain using reflection, which
will always work, but is slow compared to direct access. Alternatively, you can configure Timefold to access your domain
...
These restrictions do not apply when using Timefold with Quarkus,
...
*In Timefold, all problem facts and planning entities are plain old JavaBeans (POJOs).* Load them from a database, an XML file, a data repository, a REST service, a noSQL cloud, ... (see xref:integration/integration.adoc#integration[integration]): it doesn't matter.
...
A problem fact class does _not_ require any Timefold specific code.
...
Therefore, when dealing with a messy (denormalized) legacy system, it can sometimes be worthwhile to convert the messy domain model into a Timefold specific model first.
...
If none are provided, Timefold will attempt to auto-detect matching ``@ValueRangeProvider``s.
...
Timefold will automatically add the value `null` to the value range.
...
Timefold will go over every ``@ValueRangeProvider``-annotated method or field which does not have an ``id`` property set,
...
[NOTE]
====
Return `CountableValueRange` instead of `ValueRange` whenever possible (so Timefold knows that it's countable).
====
...
Timefold needs to extract the entity instances from the solution instance.
...
For xref:constraint-streams/constraint-streams.adoc#constraintStreams[Constraint Streams],
Timefold needs to extract the problem fact instances from the solution instance.
...
some can also be deduced automatically by Timefold.
...
In this mode, two runs in the same Timefold version will execute the same code in the same order.
...
[NOTE]
====
**If an error happens, Timefold normally fails fast**: it throws a subclass of `RuntimeException` with a detailed message to the calling code.
...
Timefold exposes metrics through https://micrometer.io/[Micrometer] which you can use to monitor the solver. Timefold automatically connects to configured registries when it is used in Quarkus or Spring Boot. If you use Timefold with plain Java, you must add the metrics registry to the global registry.

.Prerequisites
* You have a plain Java Timefold project.
...
. Open your monitoring system to view the metrics for your Timefold project. The following metrics are exposed:
+
[NOTE]
====
...
For more detailed monitoring, Timefold can be configured to monitor additional metrics at a performance cost.
...
- `SCORE_CALCULATION_COUNT` (default, Micrometer meter id: "timefold.solver.score.calculation.count"): Measures the number of score calculations Timefold performed.

- `BEST_SCORE` (Micrometer meter id: "timefold.solver.best.score.*"): Measures the score of the best solution Timefold found so far. There are separate meters for each level of the score. For instance, for a `HardSoftScore`, there are `timefold.solver.best.score.hard.score` and `timefold.solver.best.score.soft.score` meters.

- `STEP_SCORE` (Micrometer meter id: "timefold.solver.step.score.*"): Measures the score of each step Timefold takes. There are separate meters for each level of the score. For instance, for a `HardSoftScore`, there are `timefold.solver.step.score.hard.score` and `timefold.solver.step.score.soft.score` meters.
