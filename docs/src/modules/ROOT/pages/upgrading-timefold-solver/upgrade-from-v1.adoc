...
.icon:info-circle[role=yellow] `count()` and `countDistinct()` now return `Long` instead of `Integer`
[%collapsible%open]
====
`ConstraintCollectors.count()` and `ConstraintCollectors.countDistinct(...)` previously returned `Integer`.
They now return `Long`.
Any code assigning the result to an `Integer` variable or using it as an `int` must be updated to use `Long` or `long`.
====

'''

.icon:info-circle[role=yellow] `sum(ToIntFunction)` removed; use `sum(ToLongFunction)`
[%collapsible%open]
====
`ConstraintCollectors.sum(ToIntFunction)` has been removed.
Replace it with `sum(ToLongFunction)` and update the lambda accordingly, e.g. `sum(Visit::getServiceDuration)` where `getServiceDuration()` returns `int` must be changed to return `long`, or wrapped: `sum(v -> (long) v.getServiceDuration())`.
====
