...
`asConstraint()` call.
* Where the old form passed both a package and a name (`penalize(pkg, name, score)`), the package and name are merged into a single string for `asConstraint()`: `penalize(score).asConstraint(pkg + "." + name)`.
* `ConstraintBuilder.asConstraint(packageName, constraintName)` → `asConstraint(packageName + "." + constraintName)`
...
* The `Move` interface moved to `ai.timefold.solver.core.preview.api.move.Move`, and its methods have changed.
