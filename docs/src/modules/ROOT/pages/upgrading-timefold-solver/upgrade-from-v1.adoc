...
Timefold Solver 2.x brings many improvements comparing to 1.x, and some breaking changes as well.
...
Make sure to first upgrade to the latest 1.x version and only then follow this recipe to upgrade to 2.x.
...
== Automatic upgrade to Timefold Solver 2.x
...
and other mechanical changes that were not previously deprecated.
...
* `ConstraintRef.of(packageName, constraintName)` → `ConstraintRef.of(constraintId)` (package argument removed).
* `Constraint.getConstraintId()` replaced by `getConstraintRef().id()`.
* `Constraint.getConstraintName()` removed.
...
`reward(..)`, `impact(..)`.
* "Constraint name" was renamed to "Constraint ID".
* "Constraint description" and "Constraint group" were removed,
replaced by a more flexible `ConstraintMetadata`.
...
==== `SolverManager` and adjacent types have some generics removed
...
* `SolverConfigOverride<Solution_>` becomes `SolverConfigOverride` (type parameter dropped).
...
* Jackson score serializers: per-score-type packages merged into `ai.timefold.solver.jackson.api.score`.
...
==== Constraint Collector `int` variants removed

* `countLong()` renamed to `count()` (now returns `long`).
* `countDistinctLong(fn)` renamed to `countDistinct(fn)`.
* `sumLong(fn)` renamed to `sum(fn)`.
* `averageLong(fn)` renamed to `average(fn)`.
...
.icon:exclamation-triangle[role=red,set=fas] Java 21 or later required
[%collapsible%open]
====
...
.icon:exclamation-triangle[role=red,set=fas] Spring Boot 4 and Jackson 3
[%collapsible%open]
====
...
.icon:exclamation-triangle[role=red,set=fas] Chained planning variable support removed
[%collapsible%open]
====
...
.icon:exclamation-triangle[role=red,set=fas] Variable listeners removed
[%collapsible%open]
====
The variable listener mechanism has been removed.
Variable listeners were deprecated since 1.26.0 in favor of xref:domain-modeling/modeling-planning-problems.adoc#customShadowVariable[custom shadow variables].
...
.icon:exclamation-triangle[role=red,set=fas] Score analysis no longer open-source
[%collapsible%open]
====
...
* `isSolutionInitialized()` — whether the solution is fully initialized
* `constraintAnalyses()` — list of `ConstraintAnalysis` objects, one per constraint
* `summarize()` and `summarize(int)` — human-readable score breakdown
* `getConstraintAnalysis(ConstraintRef)` and `getConstraintAnalysis(String, String)` — look up a specific constraint
* `diff(ScoreAnalysis)` — compare two score analyses

The following `ScoreExplanation` methods have no direct replacement and must be removed manually:

* `getSolution()` — no replacement
* `getJustificationList()` — no replacement; use `constraintAnalyses()` and filter `ConstraintAnalysis.matches()` by justification type
* `getSummary()` — use `ScoreAnalysis.summarize()` instead
* `getConstraintMatchTotalMap()` — use `ScoreAnalysis.constraintAnalyses()` instead
...
.icon:exclamation-triangle[role=red,set=fas] Recommended assignment no longer open-source
[%collapsible%open]
====
The recommended assignment API is unchanged, but it now requires Timefold Solver Enterprise Edition on the classpath.

If you use Jackson serialization, register `TimefoldEnterpriseJacksonModule` in addition to the standard module.
====

'''

.icon:exclamation-triangle[role=red,set=fas] `Move` interface refactored
[%collapsible%open]
====
...
.icon:info-circle[role=yellow,set=fas] Planning solution diff no longer open-source
[%collapsible%open]
====
The solution diff API is unchanged, but it now requires Timefold Solver Enterprise Edition on the classpath.
====

'''

.icon:info-circle[role=yellow,set=fas] Full modularization under JPMS
[%collapsible%open]
====
...
and we have kept the same module name for 2.x.
...
.icon:info-circle[role=yellow,set=fas] Getter and setter access rules enforced
[%collapsible%open]
====
...
.icon:info-circle[role=yellow,set=fas] Constraint ID now strictly validated
[%collapsible%open]
====
Constraint IDs are now validated more strictly.
They must be non-null, non-empty, and must not contain slashes and other special characters.
If your constraint IDs do not comply, an exception will be thrown at solver initialization.
====

'''

.icon:info-circle[role=yellow,set=fas] `@ConstraintConfiguration` replaced by constraint weight overrides
[%collapsible%open]
====
The `@ConstraintConfiguration`, `@ConstraintConfigurationProvider`, and `@ConstraintWeight` annotations have been removed.
Please use xref:running-timefold-solver/library/constraint-overrides.adoc#definingAndOverridingConstraintWeights[constraint weight overrides] instead.
...
.icon:info-circle[role=yellow,set=fas] `Indictment` removed
[%collapsible%closed]
====
The `Indictment` interface has been removed entirely, along with:

* `indictWith()` on `UniConstraintBuilder`, `BiConstraintBuilder`, `TriConstraintBuilder`, and `QuadConstraintBuilder`
* `getIndictmentMap()` on `ScoreExplanation`

There is no replacement type for `Indictment`.
Remove all field and variable declarations of type `Indictment`.

To analyze per-entity score impact, use `ScoreAnalysis.constraintAnalyses()` and filter `ConstraintAnalysis.matches()` by justification.

The OpenRewrite migration recipe automatically removes `indictWith()` call sites.
====

'''

.icon:info-circle[role=yellow,set=fas] `@PlanningEntity` `pinningFilter` removed
[%collapsible%open]
====
...
and replaced it with xref:domain-modeling/modeling-planning-problems.adoc#pinnedPlanningEntities[`@PlanningPin` annotation] instead.
====

'''

.icon:info-circle[role=yellow,set=fas] `ScoreDirector` removed from public API
[%collapsible%open]
====
...
.icon:info-circle[role=yellow,set=fas] `PhaseCommand.changeWorkingSolution` signature changed
[%collapsible%open]
====
...
.icon:info-circle[role=yellow,set=fas] `SolutionPartitioner` signature changed
[%collapsible%open]
====
...
.icon:eye[set=far] `@PlanningSolution` `lookUpStrategyType` removed
[%collapsible%closed]
====
...
`LookupStrategyType` was used in xref:running-timefold-solver/multithreaded-solving.adoc#multithreadedIncrementalSolving[multi-threaded incremental solving]
...
that the solver would look up entities by their xref:domain-modeling/modeling-planning-problems.adoc#planningId[planning ID].
...
.icon:eye[set=far] `BenchmarkAggregator` moved to separate module
[%collapsible%closed]
====
...
.icon:exclamation-triangle[role=red,set=fas] Int-based score types removed
[%collapsible%open]
====
...
.icon:info-circle[role=yellow,set=fas] `Score` can no longer be uninitialized
[%collapsible%open]
====
...
.icon:info-circle[role=yellow,set=fas] `BendableScore` array type changed from `int[]` to `long[]`
[%collapsible%open]
====
...
.icon:info-circle[role=yellow,set=fas] Constraint stream `penalizeLong`, `rewardLong`, `impactLong` removed
[%collapsible%open]
====
...
.icon:info-circle[role=yellow,set=fas] Deprecated `SolverManager` `solve` methods removed
[%collapsible%open]
====
...
.icon:info-circle[role=yellow,set=fas] `SolverJobBuilder` deprecated consumer methods removed
[%collapsible%open]
====
...
Users of xref:running-timefold-solver/library/library-integration.adoc#throttlingBestSolutionEvents[solution throttling]
...
.icon:info-circle[role=yellow,set=fas] `SubList` selector configuration moved
[%collapsible%open]
====
...
.icon:info-circle[role=yellow,set=fas] `NoChangePhase` removed from XML configuration
[%collapsible%open]
====
...
.icon:info-circle[role=yellow,set=fas] `EnvironmentMode` `FAST_ASSERT` and `REPRODUCIBLE` removed
[%collapsible%open]
====
...
.icon:info-circle[role=yellow,set=fas] All code must use `RandomGenerator` instead of `Random`
[%collapsible%open]
====
...
.icon:eye[set=far] `Termination` can no longer be customized
[%collapsible%open]
====
...
.icon:eye[set=far] `Score` can no longer be customized
[%collapsible%open]
====
...
.icon:eye[set=far] `ConstraintRef` package change and constraint packages removed
[%collapsible%closed]
====
...
.icon:eye[set=far] `ConstraintMetadata` replaces string-based constraint description and group APIs
[%collapsible%closed]
====
The old `asConstraintDescribed(String name, String description)` and `asConstraintDescribed(String name, String group, String description)` methods have been removed.
Use `asConstraint(String id)` (description dropped) or implement the `ConstraintMetadata` interface and call `asConstraint(ConstraintMetadata)` to carry additional metadata.

The following methods have also been renamed or removed:

* `ConstraintRef.constraintName()` → `ConstraintRef.id()`
* `ConstraintAnalysis.constraintName()` → `ConstraintAnalysis.constraintId()`
* `Constraint.getConstraintGroup()` removed — constraint groups are no longer supported.
* `Constraint.getDescription()` returning `String` removed — use `Constraint.getConstraintMetadata()` returning `ConstraintMetadata` instead.
* `ConstraintMetaModel.getConstraintsPerGroup(...)` and `ConstraintMetaModel.getConstraintGroups()` removed.
====

'''

.icon:eye[set=far] Deprecated `ConstraintCollectors` methods removed
[%collapsible%closed]
====
...
.icon:eye[set=far] `count()` and `countDistinct()` now return `Long` instead of `Integer`
[%collapsible%open]
====
`ConstraintCollectors.count()` and `ConstraintCollectors.countDistinct(...)` previously returned `Integer`.
They now return `Long`.
====

'''

.icon:eye[set=far] `sum(ToIntFunction)` removed
[%collapsible%open]
====
`ConstraintCollectors.sum(ToIntFunction)` has been removed.
Replace it with `sum(ToLongFunction)` and update the lambda accordingly.
====

'''

.icon:eye[set=far] `AutoDiscoverMemberType` removed from `@PlanningSolution`
[%collapsible%closed]
====
...
.icon:eye[set=far] `DoubleValueRange` removed
[%collapsible%closed]
====
...
.icon:eye[set=far] `BestSolutionChangedEvent` is now an interface
[%collapsible%closed]
====
