...
This section explains how to update your planning model to use the new declarative xref:running-timefold-solver/modeling-planning-problems.adoc#customShadowVariable[custom shadow variable]
...
[#migrateVariableListenersWhyMigrate]
== Why migrate
...
This approach has been removed in Timefold Solver 2.0.0.
...
[#migrateVariableListenersMigrationSteps]
== Migration steps
...
[#migrateVariableListenersBeforeAfter]
...
[#migrateVariableListenersAdvanced]
...
Read the xref:running-timefold-solver/modeling-planning-problems.adoc#shadowSourcesPaths[full @ShadowSources reference here].
...
[source,java]
----
import java.time.LocalDateTime;// This example keeps the "travel time matrix" on the solution
public class Visit {
    private Location location;
    ...
    @ShadowSources({"previous.arrivalTime"})
    public LocalDateTime computeArrivalTime(RoutingSolution solution) {
        if (previous == null) {
            return null;
        }
        // Global travel time matrix stored on the solution
        var travelTime = solution.getTravelTime(previous.getLocation(), location);
        var previousDeparture = previous.arrivalTime.plus(solution.getDefaultDuration());
        return previousDeparture.plus(travelTime);
    }
...
[#migrateVariableListenersNext]
== Next

* xref:running-timefold-solver/modeling-planning-problems.adoc#customShadowVariable[Custom shadow variables]
* xref:running-timefold-solver/modeling-planning-problems.adoc#shadowSourcesPaths[@ShadowSources paths]