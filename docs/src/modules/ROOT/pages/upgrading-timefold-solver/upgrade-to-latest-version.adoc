...
[%collapsible%open]
====
Previously, `flattenLast` uses object identity to determine when to create or retract tuples.
This was particularly inefficient when a mapping function was used with records:
...
Since the `map` create a new interval instance every update, the `flattenLast` does a retract of the old instance followed by an insert of the new instance.
Now, `flattenLast` use object equality and `hashCode` to determine when to create or retract tuples.
For the example given above, since the different intervals are `equals` to each other, `flattenLast` will reuse the old interval and do an `update` on it.
For the vast majority of cases, this change is not noticeable.
However, if an object overrides `equals` and `hashCode` such that some property is not considered for `equals` or `hashCode`, and your constraints check that property after `flattenLast`, score corruption can occur since a different `equals` instance with a different value for that property can be used.
For example:
...
To fix this, ensure the objects returned by the iterable passed to `flattenLast` are not `equals` if any checked properties downstream are different.
For instance:
...
factory.forEach(Shift.class)
    .map(shift -> new IntervalEqualByStart(shift.getStart(), shift.getEnd()))
    .flattenLast(intervalEqualByStart -> List.of(intervalEqualByStart)
        .stream()
        .map(intervalEqualByStart -> new IntervalEqualByStartAndEnd(intervalEqualByStart.start(), intervalEqualByStart.end()))
        .toList()
    )
