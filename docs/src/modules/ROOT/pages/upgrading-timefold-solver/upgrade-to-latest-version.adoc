...
.icon:exclamation-triangle[role=red] Chained shadow variable deprecated for removal
[%collapsible%open]
====
Many years ago, we have introduced the xref:using-timefold-solver/modeling-planning-problems.adoc#planningListVariable[list variable] as a better alternative to xref:using-timefold-solver/modeling-planning-problems.adoc#chainedPlanningVariable[chained variables],
easier to both understand and implement.
We have maintained both these paradigms since then,
but in the next major version of the solver,
we will be removing support for chained shadow variables entirely.

The migration from chained variable to list variable requires some effort and it cannot be automated.
...
and to reach out to us for help should you find yourself stuck.
====
...
.icon:exclamation-triangle[role=red] `SolverJobBuilder` `with...SolutionConsumer` methods deprecated for removal
[%collapsible%open]
====
...
.icon:exclamation-triangle[role=red] Variable listeners deprecated for removal
[%collapsible%open]
====
With the introduction of xref:using-timefold-solver/modeling-planning-problems.adoc#customShadowVariable[custom shadow variables],
we have deprecated the variable listener API
and will be removing the mechanism entirely in Timefold Solver 2.0.

The migration to the new mechanism requires some effort and it cannot be automated.
