...
Luckily, defining constraints in Timefold is very flexible through the following score techniques:
...
[NOTE]
====
By default, Timefold will always assign all planning variables a planning value.
...
(even though Timefold can handle many score levels).
====
...
It is left up to a human to choose the better out of a set of best solutions (with equal scores) found by Timefold.
...
Yet, two apples are not greater than one orange (but actually equal). Pareto comparison violates the contract of the interface ``java.lang.Comparable``'s `compareTo` method, but Timefold's systems are __pareto comparison safe__, unless explicitly stated otherwise in this documentation.
====
...
Timefold does not recalculate the score of a solution if it can predict it (unless an xref:configuration/configuration.adoc#environmentMode[environmentMode assertion] is enabled).
...
*xref:constraint-streams/constraint-streams.adoc#constraintStreams[Constraint Streams] gives you this huge scalability gain without forcing you to write a complicated incremental score calculation algorithm.*
...
In xref:constraint-streams/constraint-streams.adoc#constraintStreams[constraint streams], the return type xref:constraint-streams/constraint-streams.adoc#constraintStreamsCustomizingJustificationsAndIndictments[can be customised],
...
[NOTE]
====
xref:constraint-streams/constraint-streams.adoc#constraintStreams[Constraint streams] supports constraint matches automatically, but <<incrementalJavaScoreCalculation,incremental Java score calculation>> requires
