...
`from()` behaves unexpectedly for xref:using-timefold-solver/modeling-planning-problems.adoc#nullablePlanningVariable[nullable variables].
...
xref:performance.adoc#incrementalScoreCalculation[incremental score calculation],
...
and `shiftList` contains 6 shifts, then `violationAmount` will be `1`.
