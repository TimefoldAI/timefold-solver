...
that calling `giveSolution(...)` does not trigger any xref:using-timefold-solver/modeling-planning-problems.adoc#customVariableListener[custom listener] or the xref:using-timefold-solver/modeling-planning-problems.adoc#tailChainVariable[cascading update listener].
...
To test all constraints while activating the variable listeners,
such as xref:using-timefold-solver/modeling-planning-problems.adoc#customVariableListener[custom listeners] and xref:using-timefold-solver/modeling-planning-problems.adoc#tailChainVariable[cascading update listeners],
call `giveSolution(...)` and `settingAllShadowVariables()`.
This approach ensures that all listeners are triggered,
and the constraints that depend on the related shared variables are calculated accordingly.
