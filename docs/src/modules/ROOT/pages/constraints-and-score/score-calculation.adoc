...
The methods `justifiesWith(...)` and `indictsWith(...)` validate the expected constraint justifications and indictments
mapped by the constraint definition.
The constraint justification validation expects the ID of both `queen1` and `queen2`.
Similarly, the indictment check tests if the constraint `horizontalConflict` returns both indicted queens.
...
It is also possible to provide an alternative comparison logic by implementing the `Comparable` interface.
====
