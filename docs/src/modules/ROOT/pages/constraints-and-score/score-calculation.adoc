...
When a stream depends only on properties of problem facts or entities, that stream can be precomputed to improve performance.
...
Constraint noOverlappingShifts(ConstraintFactory constraintFactory) {
    return constraintFactory.forEachUniquePair(Shift.class, Joiners.overlapping(Shift::getStart, Shift::getEnd), Joiners.equal(Shift::getEmployee))
...
This constraint will recalculate what shifts overlap and their overlap amount whenever any of their variable changes.
If Shift's `start` and `end` are not variables (genuine or shadow), and Shift's `getOverlap` does not depend on any variables, then some parts of the constraint can be precomputed to improve performance:
...
TriConstraintStream<Shift, Shift, Integer> overlappingShifts(PrecomputeFactory precomputeFactory) {
    return precomputeFactory.forEachUnfilteredUniquePair(Shift.class, Joiners.overlapping(Shift::getStart, Shift::getEnd))
        .expand((shift1, shift2) -> shift1.getOverlap(shift2));
}

Constraint noOverlappingShifts(ConstraintFactory constraintFactory) {
    return constraintFactory.precompute(this::overlappingShifts)
        .filter((shift1, shift2, overlap) -> shift1.getEmployee() != null && shift1.getEmployee() == shift2.getEmployee())
...
This will precompute both that shifts that overlap and the overlap amount.
When any of the overlapping shifts' variables changes, only the filter is re-evaluated.
That filter is never evaluated for non-overlapping shifts, since they will not be included in the overlapping shifts stream.

[NOTE]
====
Constraint streams are extremely efficient, so precomputing them may result in little change.
Always measure the move evaluation speed before and after implementing precomputing to determine if it worth it for your use case.
====

[NOTE]
====
Precomputed streams are unfiltered and will include both unassigned and inconsistent planning entities.
If only assigned entities are desired, add a filter after the precompute to check if the entities' variables are assigned.
====

[IMPORTANT]
====
A score corruption will occur if a precomputed stream depends on variables (genuine or shadow).
For example, if Shift's employee is a planning variable, then the following constraint will cause a score corruption:
