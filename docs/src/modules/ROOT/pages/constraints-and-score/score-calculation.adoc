...
you can use the `prefilter()` building block instead to avoid re-checking it.

The `prefilter()` building block is called only once per constraint match,
and will not be re-evaluted on any change.
Its use is much like `filter()`:
...
----
    private Constraint onlyPenalizeDifficultShifts(ConstraintFactory factory) {
        return factory.forEach(Shift.class)
                .prefilter(shift -> ShiftUtil.isDifficultShift(shift))
...
NOTE: If you're not sure whether to use `filter()` or `prefilter()`, use `filter()`.
`prefilter()` is an advanced feature that is only necessary in special cases to improve performance.
