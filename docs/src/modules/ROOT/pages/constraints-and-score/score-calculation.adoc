...
- `contain()`: the match on the left is a collection containing the match on the right.
- `containedIn()`: the match on the right is a collection containing the match on the left.
- `intersect()`: the collections on both sides have at least one element in common.
