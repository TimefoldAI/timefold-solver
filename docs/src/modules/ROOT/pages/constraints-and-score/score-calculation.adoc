...
you can use the `filterByFact()` building block instead to avoid re-checking it.

The `filterByFact()` building block is called only once per constraint match,
...
----
    private Constraint onlyPenalizeDifficultShifts(ConstraintFactory factory) {
        return factory.forEach(Shift.class)
                .filterByFact(shift -> ShiftUtil.isDifficultShift(shift))
...
NOTE: If you're not sure whether to use `filter()` or `filterByFact()`, use `filter()`.
`filterByFact()` is an advanced feature that is only necessary in special cases to improve performance.
