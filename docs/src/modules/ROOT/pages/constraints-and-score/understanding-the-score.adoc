...
=== Finding indicted objects

To find the planning entities and problem facts that contribute to the score, use `ScoreAnalysis.indictmentMap()`:

[source,java,options="nowrap"]
----
scoreAnalysis.indictmentMap().forEach((type, indictments) -> {
    indictments.forEach(indictment -> {
        Object indictee = indictment.indictee();
        HardSoftScore scoreImpact = indictment.score();
        ...
    });
});
----

The map is keyed by `IndictableTypeRef`, and each `IndictmentAnalysis` contains the indicted object, its total score impact,
and the constraint matches involving it. The entries are sorted by the absolute value of their total score impact.
`indictmentMap()` is populated only when the score analysis uses `ScoreAnalysisFetchPolicy.FETCH_ALL`; it is empty for the other fetch policies.

By default, the type reference uses the canonical class name and the indicted object is the same instance as the planning entity or problem fact.
To expose a different type identifier or object, implement `Indictable`:

[source,java,options="nowrap"]
----
public class PlanningEntity implements Indictable {

    @Override
    public IndictableTypeRef getIndictableTypeRef() {
        return IndictableTypeRef.of("PlanningEntity");
    }
...
When using `ScoreAnalysisFetchPolicy.FETCH_ALL`, the JSON also contains the indicted objects and their constraint matches in the `indictments` field.
