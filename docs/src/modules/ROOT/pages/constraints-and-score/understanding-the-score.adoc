...
* A full list of entities whose genuine planning variables changed between the two solutions.
Each entity is represented by a `PlanningEntityDiff` instance,
which contains the entity itself,
as well as the old and new values of its genuine planning variables.
