...
Each constraint has a constraint name, serving as a unique identifier for the constraint.
It connects the constraint weight with the constraint implementation.
*For each constraint weight, there must be a constraint implementation with the same constraint name.*
...
So every constraint weight ends up with a name.
