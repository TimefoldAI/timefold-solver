...
Pareto scoring (rarely used):: Optimize multiple objectives without weighting them against each other.
...
NOTE: Do not presume that your business knows all its score constraints in advance.
...
instead add a xref:running-timefold-solver/library/constraint-overrides.adoc[constraint configuration]
...
To instead leave some of the planning entities unassigned, apply xref:domain-modeling/common-patterns.adoc#overconstrainedPlanning[overconstrained planning].
====
...
see xref:running-timefold-solver/library/integration.adoc#integration[the integration chapter].
====
...
A `SimpleScore` has a single `long` value, for example ``-123``.
...
A variant score type `SimpleBigDecimalScore` uses a `BigDecimal` value instead of an `long` value.


[#hardSoftScore]
=== `HardSoftScore` (Recommended)

A `HardSoftScore` has a hard `long` value and a soft `long` value, for example ``-123hard/-456soft``.
...
A variant score type `HardSoftBigDecimalScore` uses `BigDecimal` values instead of `long` values.


[#hardMediumSoftScore]
=== `HardMediumSoftScore`

A `HardMediumSoftScore` which has a hard `long` value, a medium `long` value and a soft `long` value, for example ``-123hard/-456medium/-789soft``.
...
A variant score type `HardMediumSoftBigDecimalScore` uses `BigDecimal` values instead of `long` values.
...
It has an array of hard `long` values and an array of soft `long` values,
...
A variant score type `BendableBigDecimalScore` uses `BigDecimal` values instead of `long` values.
...
xref:constraints-and-score/score-calculation.adoc[Constraint Streams API]:: Implement each constraint as a separate Constraint Stream. Fast and scalable.
...
xref:constraints-and-score/score-calculation.adoc#easyScoreCalculation[Easy score calculator] (not recommended):: Implement all constraints together in a single method. Does not scale. Does not support xref:constraints-and-score/understanding-the-score.adoc[score explanations].
...
[IMPORTANT]
====
The score calculation must be deterministic and free of side-effects.
...
Timefold Solver does not recalculate the score of a solution if it can predict it (unless an xref:running-timefold-solver/solver-diagnostics.adoc#environmentMode[environmentMode assertion] is enabled).
...
To update planning entities when the planning variable change, use xref:domain-modeling/modeling-planning-problems.adoc#shadowVariable[shadow variables] instead.
====
...
When you put the xref:running-timefold-solver/solver-diagnostics.adoc#environmentMode[`environmentMode`] in `FULL_ASSERT` (or ``STEP_ASSERT``),
