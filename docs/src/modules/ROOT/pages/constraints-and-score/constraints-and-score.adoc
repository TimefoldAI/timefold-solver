...
of two constraint streams of the same <<constraintStreamsCardinality,cardinality>>.
...
If the two constraint stream parents share tuples, which happens eg.
when the streams being concatenated come from the same source of data,
