...
:score-class: HardMediumSoftScore
...
timefold.model.name=my-model
timefold.model.contact.email=info@example.com
timefold.model.contact.name=Your Name
timefold.model.contact.url=https://example.com
----

`timefold.model.termination.spent-limit` sets the default maximum time the solver will run.
The value uses the https://en.wikipedia.org/wiki/ISO_8601#Durations[ISO 8601 duration format]: `PT30S` means 30 seconds.
