...
- Configures this class to be (de-)serialized as part of the `ModelInput` and `ModelOutput` of the xref:../../rest-api.adoc[REST API].
...
Detailed instruction on how to set this up more correctly will be found in the xref:../../modeling-changes.adoc[solver part of the documentation].
...
This is the most basic version of the interface. The SDK will automatically create multiple xref:../../rest-api.adoc[REST API endpoints] for optimization actions.
...
We discuss these in more depth in the xref:../../rest-api.adoc[REST API] section.
...
This call with return the standard xref:../../rest-api.adoc#solvingResponse[response structure].
[source,json]
----
...
** How to configure your xref:../../rest-api.adoc[REST API] with validations, custom endpoints, etc.