...
NOTE: Do not include an API version in `@Path`. The SDK parent POM configures `quarkus.rest.path=${model.api.version}` (default: `v1`), which Quarkus automatically prepends to every `@Path` at runtime.
