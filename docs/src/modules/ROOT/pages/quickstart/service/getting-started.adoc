...
:description: Getting started with the [MDK]
:doctype: book
:sectnums:
:icons: font
...
NOTE: TODO: more explainer here. The [MDK] is currently only available in Java and is built upon Quarkus.

IMPORTANT: The [MDK] way of building things is currently in preview.
...
- Configures this class to be (de-)serialized as part of the `ModelInput` and `ModelOutput` of the xref:architecture/rest-api.adoc[REST API].
...
[source,java]
----
include::_examples/Timetable.java[]
----
--
====
...
Detailed instruction on how to set this up more correctly will be found in the xref:architecture/solver.adoc[solver part of the documentation].

== REST API

The standard way to interact with the optimization service is through a REST API.

Using the [MDK], most of the REST Endpoints will be automatically generated thanks to the use of the `ModelInput`, `ModelOutput` and `SolverModel` interfaces.
...
[source,java]
----
include::_examples/TimetableResource.java[]
----
--
====

This is the most basic version of the interface. The SDK will automatically create multiple xref:architecture/rest-api.adoc[REST API endpoints] for optimization actions.
...
[source,json]
----
include::_examples/input-response.json[]
----
...
----
curl -X 'GET' \
  'http://localhost:8080/v1/timetables/f401a609-403b-48c4-9487-0baf287e4f72' \
  -H 'accept: application/json'
----

This call with return the standard xref:architecture/rest-api.adoc#solvingResponse[response structure].
[source,json]
----
include::_examples/output-response.json[]
----
...
** How to set up the underlying xref:architecture/solver.adoc[Timefold Constraint Solver].
** How to configure your xref:architecture/rest-api.adoc[REST API] with validations, custom endpoints, etc.