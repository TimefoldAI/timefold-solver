...
Timefold Solver uses the variable type to connect to a xref:running-timefold-solver/modeling-planning-problems.adoc#planningValueRangeProvider[value range provider]
...
Read xref:design-patterns/domain-modeling.adoc#domainModelingGuide[the domain modeling guidelines] to avoid common pitfalls.
====
