...
xref:running-timefold-solver/configuration.adoc#annotationAlternatives[Annotating the field] instead of the property works too:
...
[NOTE]
====
When xref:running-timefold-solver/library-integration.adoc[using Timefold Solver as a library], the `SolverModel` interface is not available.
