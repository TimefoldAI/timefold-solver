...
:page-aliases: shadow-variable/shadow-variable.adoc, using-timefold-solver/modeling-planning-problems.adoc
:doctype: book
:sectnums:
:icons: font
...
If instead you are looking for guidance on how to create a good domain model, read the xref:domain-modeling/domain-modeling.adoc[domain modeling guide].


[NOTE]
====
If you have never tried Timefold Solver before, we recommend following our xref:quickstart/overview.adoc[Getting Started guides] for a practical introduction.
====
...
image:domain-modeling/modeling-planning-problems/classDiagramSchoolTimetabling.svg[align="center"]
////
...
*In Timefold Solver, all elements of the planning domain model are plain objects.* Load them from a database, an XML file, a data repository, a REST service, a noSQL cloud, ... (see xref:running-timefold-solver/library/integration.adoc#integration[integration]): it doesn't matter.
...
For more detailed guidance on when to use which variant, read the xref:domain-modeling/domain-modeling.adoc#domainModelingGuide[domain modeling guide].
...
|===
| | xref:planningVariable[@PlanningVariable] | xref:planningListVariable[@PlanningListVariable]

| *Assignment model*
| One value per entity
| Ordered list of values per entity

| *Values shared across entities*
| Yes, the same value can be assigned to multiple entities
| No, each value belongs to exactly one entity
...
| *Typical use case*
| Assign a room to a lesson, assign an employee to a shift
| Route customers for a vehicle, sequence tasks for a worker
...
image::domain-modeling/modeling-planning-problems/shadowVariableExample.svg[align="center"]
...
Depending on your use of xref:planningVariable[@PlanningVariable] or the xref:planningListVariable[@PlanningListVariable]
several built-in shadow variables are provided.
These are documented in the details section of the respective genuine planning variables.
...
(such as xref:running-timefold-solver/multithreaded-solving.adoc#multithreadedIncrementalSolving[multi-threaded incremental solving]
and xref:responding-to-change/real-time-planning.adoc[real-time planning]),
...
NOTE: When Timefold Solver is xref:running-timefold-solver/library/integration.adoc#integration[integrated in Quarkus or Spring], the XML file is optional.
...
See xref:optimization-algorithms/move-selector-reference.adoc#sortedSelection[sorted selection] for more information.
...
xref:running-timefold-solver/library/configuration.adoc#annotationAlternatives[Annotating the field] instead of the property works too:
...
see xref:#planningListVariable[planning list variable].
====
...
See the xref:domain-modeling/common-patterns.adoc#overconstrainedPlanning[overconstrained planning] guide for more information.
====


Repeated planning
(especially xref:responding-to-change/real-time-planning.adoc[real-time planning])
...
=== Pinning a planning variable (`@PlanningPin`)
...
use xref:responding-to-change/real-time-planning.adoc#problemChange[ProblemChange].
...
The object model matches the natural language description of the problem:
...
NOTE: Only a single xref:domain-modeling/modeling-planning-problems.adoc#planningListVariable[@PlanningListVariable] is supported per solution.
...
In an xref:domain-modeling/common-patterns.adoc#overconstrainedPlanning[over-constrained use case],
...
=== Pinning a list variable (`@PlanningPinToIndex`)

There are cases where pinning only a part of a planning list variable is necessary.
...
<<pinnedPlanningEntities,`@PlanningPin`>> needs to be used on the entity itself.
...
It must never contain duplicate values,
as defined by the `equals()` method of the value's type.
It must also not contain the value ``null``,
...
The `@PlanningSolution` implementation has a method that returns a `Collection` (or a ``ValueRange``).
...
This also helps reduce the overhead of the xref:domain-modeling/modeling-planning-problems.adoc#cloningASolution[solution cloning],
...
Instead of a ``Collection``, you can also return ``ValueRange``, built by the ``ValueRangeFactory``:
...
----
    @ValueRangeProvider
    public ValueRange<Integer> getStockAmountRange() {
...
If the model uses a xref:domain-modeling/modeling-planning-problems#planningListVariable[list variable],
...
When a solution model combines xref:domain-modeling/modeling-planning-problems.adoc#planningVariable[basic] and xref:domain-modeling/modeling-planning-problems.adoc#planningListVariable[list] planning variables,
...
Their applicability and usage varies depending on your use of xref:basicVariableShadowVariable[@PlanningVariable]
or the xref:listVariableShadowVariables[@PlanningListVariable].
...
- The supplier method may optionally accept a single parameter of the solution class type,
which allows access to the solution state without the need of propagating it to the genuine or shadow entities.
...
|"variableName"
|Refers to a variable (genuine or shadow) on the same planning entity. A planning list variable cannot be used directly; use List Variable Element Access instead.
...
|List Variable Element Access
|"listVar[].shadowVarName"
|Refers to a declarative shadow variable (`shadowVarName`) on each element currently assigned to a `@PlanningListVariable` (`listVar`) on the entity. The supplier is recomputed when the list variable or any element's `shadowVarName` changes.
|===
...
The supplier method may include only one optional parameter, the solution class. Here’s an example to show this.
...
public class Job {
    ...
    @ShadowVariable(supplierName="processStartDate")
    private boolean startDateOnHoliday;

    @ShadowSources("startDate")
    public boolean processStartDate(PackagingSchedule schedule) {
        if (startDate == null) {
            return false;
        } else {
            return schedule.isHoliday(startDate);
        }
    }
...
[#customShadowVariableMultipleVariables]
==== Updating multiple fields at once

A supplier method can only update the single field it is declared on via `@ShadowVariable(supplierName = ...)`.
There is no way for one supplier method to directly update several independent shadow variable fields.

If the fields don't need to be recomputed together, chain them instead: let one shadow variable's supplier
declare another shadow variable as one of its `@ShadowSources`, the way `endDate` sources from `startDate`
in the earlier example on this page. Each supplier still only ever sets its own field, but Timefold
reruns only the suppliers whose declared sources actually changed, so unrelated downstream fields
aren't recomputed unnecessarily.

However, when several pieces of shadow state are always computed together and change at the same time,
it's often simpler, and cheaper, since the underlying calculation only has to run once, to group them
into a single wrapper object, such as a `record`, and make that wrapper itself the shadow variable.
The supplier method then returns a new instance of the wrapper every time it runs,
and the individual pieces of state are read off that wrapper through its accessor methods.
This avoids repeating the same calculation in multiple supplier methods
and keeps a set of related fields consistent with one another.
...
[source,java,options="nowrap"]
----
public record JobState(LocalDate readyDate, LocalDate startDate, LocalDate endDate, int totalDurationInDays) {
}
...
public class Job {
    ...
    @ShadowVariable(supplierName = "jobStateSupplier")
    private JobState jobState;

    @ShadowSources("previous.jobState")
    public JobState jobStateSupplier() {
        // 1. Ready date: the previous job's end date, or null if this job is first in the chain.
        LocalDate readyDate = previous == null ? null : previous.jobState.endDate();

        // 2. Start date: can't start before the previous job ends, nor before this job's earliest start date.
        LocalDate startDate = readyDate == null || readyDate.isBefore(earliestStartDate) ? earliestStartDate : readyDate;

        // 3. End date
        LocalDate endDate = startDate == null ? null : startDate.plusDays(durationInDays);

        // 4. Total duration in days, accumulated along the chain of previous jobs
        int totalDurationInDays = (previous == null ? 0 : previous.jobState.totalDurationInDays()) + durationInDays;

        return new JobState(readyDate, startDate, endDate, totalDurationInDays);
    }
}
----
====

Here, `readyDate`, `startDate`, `endDate` and `totalDurationInDays` are all recomputed together in `jobStateSupplier()` and packaged into a single `JobState` instance.
Any code that needs one of these values reads it off the `jobState` field, for example `job.getJobState().endDate()`.
...
When a declarative shadow variable is inconsistent, the solver's reaction depends on whether any entity in the model declares a deprecated `@ShadowVariablesInconsistent` field:
...
- If no entity declares such a field, the shadow variable update is canceled and the solution's score will have a negative xref:constraints-and-score/overview.adoc#structuralScore[structural score],
...
IMPORTANT: see xref:upgrading-timefold-solver/migration-guides/shadow-variables-inconsistent-to-structural-score.adoc[how to migrate from `@ShadowVariablesInconsistent` to structural scores] for a guide on how to update your domain model to remove the deprecated `@ShadowVariablesInconsistent` fields.
...
image::domain-modeling/modeling-planning-problems/alignmentShadowVariable.png[align="center"]
...
A solution class is annotated with `@PlanningSolution` and implements the `SolverModel` interface.
It holds all problem facts, planning entities and a score.
For example, a `Timetable` class holds a list of all timeslots, all rooms and all `Lesson` instances:
...
public class Timetable implements SolverModel<HardSoftScore> {
    ...
    // other Getters/Setters/Constructors
}
----
...
[TIP]
====
If you don't need to control the score type, extend `AbstractSimpleModel` instead.
It implements `SolverModel<HardMediumSoftScore>` and includes the `@PlanningScore` field,
so you only need to define your problem facts and planning entities:
...
[NOTE]
====
When xref:running-timefold-solver/library/library-integration.adoc[using Timefold Solver as a library], the `SolverModel` interface is not available.
Your `@PlanningSolution` class is a plain Java class with no framework interface to implement.
====
...
If there are duplicate entities, the solver will fail-fast at startup.
...
image::domain-modeling/modeling-planning-problems/solutionCloning.png[align="center"]
