...
A shadow variable is a field that changes during solving, but whose value can always be derived from genuine planning variables without ambiguity. For example, the arrival time at a delivery stop can be calculated from the sequence of stops assigned to a vehicle and the known travel times between them.
Color shadow relationships (or fields) purple.
+
[NOTE]
====
