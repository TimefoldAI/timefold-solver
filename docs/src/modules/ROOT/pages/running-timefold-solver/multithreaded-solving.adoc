...
xref:#multithreadedIncrementalSolving[Multi-threaded incremental solving]:: Solve 1 dataset with multiple threads without sacrificing xref:constraints-and-score/performance.adoc#incrementalScoreCalculationPerformance[incremental score calculation].

xref:#partitionedSearch[Partitioned search]:: Split 1 dataset in multiple parts and solve them independently.
...
* Use the xref:running-timefold-solver/benchmarking-and-tweaking.adoc#benchmarker[Benchmarker] during development to determine the algorithm that is the most appropriate on average.

Multitenancy:: Solve different datasets in parallel. The xref:running-timefold-solver/library/library-integration.adoc[`SolverManager`] can help with this.

image::running-timefold-solver/multithreaded-solving/multiThreadingStrategies.png[align="center"]
...
by xref:domain-modeling/modeling-planning-problems.adoc#planningId[adding a `@PlanningId` annotation]
...
A `moveThreadCount` of `4` xref:frequently-asked-questions.adoc#sizingHardwareAndSoftware[saturates almost 5 CPU cores].
...
image::running-timefold-solver/multithreaded-solving/mapReduceIsTerribleForTsp.png[align="center"]
...
Also xref:domain-modeling/modeling-planning-problems.adoc#planningId[add a `@PlanningId` annotation]
on every planning entity class and planning value class.
...
[IMPORTANT]
====
A xref:running-timefold-solver/solver-diagnostics.adoc#logging[logging level] of `debug` or `trace` causes congestion in multi-threaded Partitioned Search
...
add the `solutionPartitionerCustomProperties` element and use xref:running-timefold-solver/library/configuration.adoc#customPropertiesConfiguration[custom properties]:
...
As explained in xref:frequently-asked-questions.adoc#sizingHardwareAndSoftware[sizing hardware and software],
...
image::running-timefold-solver/multithreaded-solving/partitionedSearchThreading.png[align="center"]
