...
The `run` configuration has five fields:
...
- `options`: An optional map of string values consumed by the active execution profiles, for example the `seed` option read by the fixed random seed profile.
