...
The SDK parent POM sets `quarkus.rest.path=${model.api.version}` (default: `v1`), which Quarkus prepends to every `@Path` at runtime.
So for `@Path("/timetables")` with the default version, `<root>` is `/v1/timetables`.

NOTE: Do not include the version in your `@Path` annotation. The SDK handles it automatically via `quarkus.rest.path`.
Including it (e.g. `@Path("/v1/timetables")`) results in a doubled prefix (`/v1/v1/timetables`) when deployed to the platform.
...
For the default version (`v1`) this is `/v1/demo-data`.

- `GET /v1/demo-data`: Retrieve all available demo dataset ids.
- `GET /v1/demo-data/\{demoDataId\}`: Retrieve the demo dataset with the given identifier
