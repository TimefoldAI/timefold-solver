...
Annotate a `long` field to make it a weight multiplier for that constraint.
...
provided no overrides at all — `modelConfig.overrides()`, may be `null`.
