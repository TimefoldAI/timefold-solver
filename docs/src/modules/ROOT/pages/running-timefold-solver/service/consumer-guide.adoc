...
This guide is for developers who want to *consume* a service built with Timefold Solver built using the `service` module.
...
curl http://localhost:8080/v1/timetables/demo-data/BASIC

# Pipe it directly into a solve request
curl -X POST http://localhost:8080/v1/timetables \
  -H "Content-Type: application/json" \
  -d "$(curl -s http://localhost:8080/v1/timetables/demo-data/BASIC)"
----
...
Submits a planning problem for optimization.
The service starts solving asynchronously and returns a dataset ID immediately.

=== Request structure

The request body is a JSON envelope with two required top-level fields: `config` and `modelInput`.
...
| `termination.spentLimit`
| Maximum wall-clock duration (ISO 8601, e.g. `PT5M` for 5 minutes).
If omitted, platform limits apply.

| `termination.unimprovedSpentLimit`
| Stops solving if no score improvement is found within this duration.
If omitted, the xref:../../optimization-algorithms/overview.adoc#diminishedReturnsTermination[diminished returns termination] is used — this is the recommended default.
...
[TIP]
====
Prefer leaving `unimprovedSpentLimit` unset and relying on diminished returns termination.
It adapts to different problem sizes and hardware automatically,
so you get good results without tuning a fixed timeout per deployment.
====

=== Response

The service responds with HTTP 200 and the initial solving response envelope.
The key field to capture is `metadata.id` — you will need it to poll for results.
...
----
{
  "metadata": {
    ...
    "name": "my-run",
    "solverStatus": "SOLVING_SCHEDULED", <2>
    "score": null,
...
<2> The solver is queued and will start soon.
...
The response structure is identical to the submit response.

=== Solver status lifecycle

The `metadata.solverStatus` field progresses through the following states:
...
| `SOLVING_SCHEDULED`
| The run is queued and waiting for a solver thread to become available.
...
| `NOT_SOLVING`
| The run was terminated early via `DELETE /<root>/{id}`.
|===

The run is complete once `solverStatus` is `SOLVING_COMPLETED`, `SOLVING_FAILED`, or `NOT_SOLVING`.
...
| Stream closes automatically when the run reaches a terminal state.
|===
...
while true; do
  ...
  case $STATUS in
    SOLVING_COMPLETED|SOLVING_FAILED|NOT_SOLVING)
...
The service sends a `Metadata` JSON event each time the run's status changes.
The stream closes automatically once the run reaches a terminal state (`SOLVING_COMPLETED`, `SOLVING_FAILED`, or `SOLVING_INCOMPLETE`).
...
The endpoint accepts an optional `?status=` query parameter to receive events only for the specified statuses.
...
A score with `0hard` means all hard constraints are satisfied — the solution is feasible.
...
Stops an active run and returns the best solution found so far.
After termination, the `solverStatus` is `NOT_SOLVING`.
...
Returns all datasets currently known to the service, including their metadata and current status.
Use this to build a dashboard or monitor multiple concurrent runs.
