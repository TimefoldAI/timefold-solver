...
The service stops emitting events once the run reaches a final solving state (`SOLVING_COMPLETED`, `SOLVING_FAILED`, or `SOLVING_INCOMPLETE`); close the connection client-side after receiving the terminal event.
If the run is already in a final state (including `DATASET_INVALID`) when you connect, the service returns HTTP 410 Gone.
