...
| `options`
| Optional map of string values consumed by the active execution profiles, such as the `seed` option for the fixed random seed profile.
|===
...
If no value is provided, the default diminished returns termination will apply. If set, stepCountLimit and moveCountLimit must be empty.
...
- `moveCountLimit` sets the maximum number of moves evaluated by the solver for solving a dataset.
