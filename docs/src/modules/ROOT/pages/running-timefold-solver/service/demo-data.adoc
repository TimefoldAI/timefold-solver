...
Two GET endpoints are generated for retrieving a demo dataset, differing in response shape:

- `GET /demo-data/\{demoDataId\}` returns the full Solving Request, wrapping your `ModelInput` in the `config`/`modelInput` envelope described in xref:./rest-api.adoc#requestResponseStructure[Request and response structure]. `POST` it straight to your `ModelRest` resource to start solving.
- `GET /demo-data/\{demoDataId\}/input` returns just the `ModelInput` object, without the envelope. Use this if you already have your own `config` and only need the model input.
