...
Any computation that happens inside a constraint is repeated on every score calculation.
If that computation involves a remote call, a database query, or a complex algorithm,
it will dominate solving time and make the solver unusably slow.
