...
and it can make solving drastically slower than the non-assert modes,
because it recalculates the score after every step to detect corruption.
...
**In development**::
 - Use the `PHASE_ASSERT` mode to catch bugs early, but be aware that it makes solving drastically slower than the non-assert modes.
