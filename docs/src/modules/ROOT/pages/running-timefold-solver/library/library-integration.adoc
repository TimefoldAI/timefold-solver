...
:page-aliases: using-timefold-solver/running-the-solver.adoc, running-timefold-solver/library-integration.adoc
:doctype: book
:sectnums:
:icons: font

When xref:running-timefold-solver/service/overview.adoc[running as a service], the framework manages the `Solver` and `SolverManager` on your behalf,
...
[NOTE]
====
The instance passed to `solve(solution)` is cloned by the Solver. Do not treat it as the best solution.
Use the returned instance instead, which is most likely xref:domain-modeling/modeling-planning-problems.adoc#cloningASolution[a planning clone] of the input.
====

NOTE: The input may be partially or fully initialized, which is common in xref:responding-to-change/real-time-planning.adoc[repeated planning].
...
In xref:running-timefold-solver/library/quarkus.adoc#integrationWithQuarkus[Quarkus] and xref:running-timefold-solver/library/spring-boot.adoc#integrationWithSpringBoot[Spring Boot],
...
xref:running-timefold-solver/library/configuration.adoc[a configuration step] is necessary.
...
[NOTE]
====
The solver also permits the configuration of multiple solver managers with distinct settings in xref:running-timefold-solver/library/quarkus.adoc#integrationWithQuarkusMultipleResources[Quarkus] or xref:running-timefold-solver/library/spring-boot.adoc#integrationWithSpringBootMultipleResources[Spring Boot].
====
