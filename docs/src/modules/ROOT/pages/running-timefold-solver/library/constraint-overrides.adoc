...
:page-aliases: constraints-and-score/constraint-configuration.adoc, running-timefold-solver/constraint-overrides.adoc
:doctype: book
:sectnums:
:icons: font
...
* You use Timefold Solver's xref:running-timefold-solver/library/quarkus.adoc#integrationWithQuarkus[Quarkus integration],
* you use Timefold Solver's xref:running-timefold-solver/library/spring-boot.adoc#integrationWithSpringBoot[Spring Boot integration],
* or you directly included the `timefold-solver-jackson` module in your project.

Overrides don't natively deserialize from JSON back to Java objects.
...
and disable some of them using xref:#applyingOverrides[overrides].
