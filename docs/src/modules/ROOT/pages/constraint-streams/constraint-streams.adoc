...
Timefold currently does not support constraint stream cardinalities higher than 4.
...
[NOTE]
====
Timefold does not provide any tuple implementations out of the box.
...
`from()`, `fromUnfiltered()` and `fromUniquePair()` are now deprecated and will be removed in a future major version of Timefold.
====

[[constraintStreamsPenaltiesRewards]]
=== Penalties and rewards

The purpose of constraint streams is to build up a xref:score-calculation/score-calculation.adoc#whatIsAScore[score] for a xref:configuration/configuration.adoc#planningProblemAndPlanningSolution[solution].
...
One of important Timefold features is its ability to xref:score-calculation/score-calculation.adoc#explainingTheScore[explain the score] of solutions it produced through the use of justifications and indictments.
...
However, both of the key mapping functions are calculated independent of the other,
...
[NOTE]
====
Timefold does not provide any `Pair`, `Triple` or `Quadruple` implementation out of the box.
====
...
Constraint streams comes in one flavor:

* `BAVET` (default): an incremental implementation that uses hash indexing techniques.
