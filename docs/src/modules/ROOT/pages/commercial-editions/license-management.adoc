...
NOTE: You can change who holds the *admin* role at any time; see xref:commercial-editions/license-management.adoc#_change_a_users_role_or_remove_a_user[Change a user's role or remove a user].
...
The invitee receives an email with an acceptance link that is valid for 14 days.
