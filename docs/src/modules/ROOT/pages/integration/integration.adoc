...
Therefore, the problem fact classes reference the planning solution class, which implies that when the solution is xref:using-timefold-solver/modeling-planning-problems.adoc#cloningASolution[planning cloned], they need to be cloned too.
...
To use Timefold Solver with Quarkus, read the xref:quickstart/quarkus/quarkus-quickstart.adoc#quarkusJavaQuickStart[Quarkus Java quick start].
...
=== Timefold configuration properties
...
`SolverManager`, `SolutionManager`, `ScoreManager`, and `ConstraintVerifier`.
...
----
<1> The following score types are available: `SimpleScore`, `SimpleLongScore`, `SimpleBigDecimalScore`, `HardSoftScore`, `HardSoftLongScore`, `HardSoftBigDecimalScore`, `HardMediumSoftScore`, `HardMediumSoftLongScore`, `HardMediumSoftBigDecimalScore`, `BendableScore`, `BendableLongScore`, and `BendableBigDecimalScore`.
--
Kotlin::
+
--
...
[NOTE]
====
Consider using xref:#integrationWithQuarkusMultiStage[Multi-stage planning] for multiple solver configurations instead of manually creating resources.
====

[#integrationWithQuarkusMultiStage]
=== Multi-stage planning

Quarkus multi-stage planning allows for defining different solver settings or even wholly distinct planning problems in the same
...
Following properties are supported per each solver:

:property_prefix: quarkus.
include::config-solver-properties.adoc[]
...
[NOTE]
====
When implementing multi-stage planning, it's important to note that only the `SolverManager` resource can be injected.
====
...
Add two XML files containing the related planning configuration.
[tabs]
====
...
<solver xmlns="https://timefold.ai/xsd/solver" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance"
  ...
  <scoreDirectorFactory>
    <constraintProviderClass>
        org.acme.schooltimetabling.solver.TeacherConstraintProvider
    </constraintProviderClass>
  </scoreDirectorFactory>
</solver>
----
...
<solver xmlns="https://timefold.ai/xsd/solver" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance"
  ...
  <scoreDirectorFactory>
    <constraintProviderClass>
        org.acme.schooltimetabling.solver.TimetableConstraintProvider
    </constraintProviderClass>
  </scoreDirectorFactory>
</solver>
----
...
Configure the solvers with the specific XML files in the `application.properties` file:
...
public class TeacherConstraintProvider implements ConstraintProvider {
    ...
}
...
class TeacherConstraintProvider : ConstraintProvider {
    ...
}
...
[NOTE]
====
Consider using xref:#integrationWithSpringBootMultiStage[Multi-stage planning] for multiple solver configurations instead of manually creating resources.
====

[#integrationWithSpringBootMultiStage]
=== Multi-stage planning

Spring Boot multi-stage planning allows for defining different solver settings or even wholly distinct planning problems in the same
application. Timefold identifies each setting and provides a specific managed resource, `SolverManager`.
...
[source,java,options="nowrap"]
----
module ai.timefold.solver.cloudbalancing {
    requires ai.timefold.solver.core;
    ...

    opens ai.timefold.solver.examples.cloudbalancing; // Solver configuration
    opens ai.timefold.solver.examples.cloudbalancing.domain; // Domain classes
    opens ai.timefold.solver.examples.cloudbalancing.score; // Constraints
    ...
}
----
...
** The human planner tweaks the constraint weights of the xref:constraints-and-score/constraint-configuration.adoc[constraint configuration] in a UI, as the business priorities change over time.
...
**** Adding more heap space increases the xref:constraints-and-score/performance.adoc#scoreCalculationSpeed[score calculation speed].
...
** Improving CPU speed directly increases the xref:constraints-and-score/performance.adoc#scoreCalculationSpeed[score calculation speed].
...
** During the `solve()` method, the CPU power will max out until it returns (except in xref:responding-to-change/responding-to-change.adoc#daemon[daemon mode] or if your xref:optimization-algorithms/optimization-algorithms.adoc#SolverEventListener[SolverEventListener] writes the best solution to disk or the network).
...
*** However, 24 active solver threads with eight cores will cause each solver's xref:constraints-and-score/performance.adoc#scoreCalculationSpeed[score calculation speed] to be three times slower, causing a big performance loss.
