...
switch to xref:../enterprise-edition/enterprise-edition.adoc[Enterprise Edition].
The initial steps configure the enterprise repository and its dependencies.
Then, the `application.properties` is updated with the multithread setting.

[NOTE]
====
The multithread feature is a commercial feature of Timefold Solver Enterprise Edition.
====

First, add the following repository to your project:
...
Configure all module versions by updating the project with the following dependency:
...
Let's take the following benchmark comparison chart for the xref:../quickstart/quarkus-vehicle-routing/quarkus-vehicle-routing-quickstart.adoc[Vehicle Routing Problem].

image::integration/vehicleRoutingBenchmark.png[align="center"]

The image illustrates the speed of score calculation with different numbers of threads running for 10 minutes each.
...
The result of the calculations are described in the following table:
...
The following image shows the score calculation speed when running the Maintenance Scheduling Problem.

image::integration/maintenanceSchedulingBenchmark.png[align="center"]
