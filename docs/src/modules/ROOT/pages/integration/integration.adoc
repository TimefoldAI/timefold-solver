...
Timefold's input and output data (the planning problem and the best solution) are plain old JavaBeans (POJOs), so integration with other Java technologies is straightforward.
For example:
...
[NOTE]
====
Do not confuse JPA's `@Entity` annotation with Timefold's `@PlanningEntity` annotation.
They can appear both on the same class:

[source,java,options="nowrap"]
----
@PlanningEntity // Timefold annotation
...
Therefore, the problem fact classes reference the planning solution class, which implies that when the solution is xref:configuration/configuration.adoc#cloningASolution[planning cloned], they need to be cloned too.
Use an `@DeepPlanningClone` on each such problem fact class to enforce that:

[source,java,options="nowrap"]
----
@PlanningSolution // Timefold annotation
@Entity // JPA annotation
...
[source,java,options="nowrap"]
----
@DeepPlanningClone // Timefold annotation: Force the default planning cloner to planning clone this class too
@Entity // JPA annotation
public class Room {
...
Add a dependency to the `timefold-solver-jsonb` jar and use `TimefoldJsonbConfig` to create a `Jsonb` instance:
...
The `TimefoldJsonbConfig` fixes that, by using adapters including `BendableScoreJsonbAdapter`, `HardSoftScoreJsonbAdapter`, etc.
...
To use Timefold with Quarkus, read the xref:quickstart/quarkus/quarkus-quickstart.adoc#quarkusJavaQuickStart[Quarkus Java quick start].
...
To use Timefold on Spring Boot, add the `timefold-solver-spring-boot-starter` dependency
...
When using Timefold from code on the modulepath (Java 9 and higher),
...
Otherwise Timefold can't reach those classes or files, even if they are exported.


[[integrationWithHumanPlanners]]
== Integration with human planners (politics)

A good Timefold implementation beats any good human planner for non-trivial datasets.
Many human planners fail to accept this, often because they feel threatened by an automated system.

But despite that, both can benefit if the human planner becomes the supervisor of Timefold:
...
* *The human planner is always in control of Timefold.*
...
Because they are xref:repeated-planning/repeated-planning.adoc#pinnedPlanningEntities[pinned], Timefold does not change them: it optimizes the planning around the enforcements made by the human.
If the human planner pins down all planning variables, he/she sidelines Timefold completely.
...
Before sizing a Timefold service, first understand the typical behaviour of a `Solver.solve()` call:

image::integration/sizingHardware.png[align="center"]

Understand these guidelines to decide the hardware for a Timefold service:

* **RAM memory**: Provision plenty, but no need to provide more.
** The problem dataset, loaded before Timefold is called, often consumes the most memory. It depends on the problem scale.
...
*** If this is a problem, review the domain class structure: remove classes or fields that Timefold doesn't need during solving.
*** Timefold usually has up to three solution instances: the internal working solution, the best solution and the old best solution (when it's being replaced). However, these are all a xref:configuration/configuration.adoc#cloningASolution[planning clone] of each other, so many problem fact instances are shared between those solution instances.
...
** Timefold is not a web server: a solver thread does not block (unlike a servlet thread), each one fully drains a CPU.
...
* JDK
** Version: Our benchmarks have consistently shown improvements in performance when comparing new JDK releases with their predecessors. It is therefore recommended using the latest available JDK. If you're interested in the performance comparisons of Timefold running of different JDK releases, you can find them in the form of blog posts in https://timefold.ai/blog/[our blog archive].
** Garbage Collector: ParallelGC can be potentially between 5% and 35% faster than G1GC (the default). Unlike web servers, Timefold needs a GC focused on throughput, not latency. Use `-XX:+UseParallelGC` to turn on ParallelGC.
