...
[NOTE]
====
Timefold Solver Spring Boot Starter only supports Spring Boot version 4.0.x and up.
====
...
[NOTE]
====
Timefold uses Jackson 2 for serialization,
and the xref:https://github.com/spring-projects/spring-boot/wiki/Spring-Boot-4.0-Migration-Guide#jackson-2-compatibility[Spring integration] also relies on that version of Jackson.
Ensure your application includes the preferred mapper in the configuration:
