...
Enable xref:commercial-editions/commercial-editions.adoc#constraintProfiling[constraint profiling] to identify the constraints taking the most time in score calculation and thus might be worth optimizing.
...
Enable xref:commercial-editions/commercial-editions.adoc#multithreadedIncrementalSolving[multithreaded incremental solving] for a single problem, which increases CPU consumption.
Defaults to `NONE`.

{property_prefix}timefold.solver.{solver_name_prefix}nearby-distance-meter-class::
Enable the xref:m#nearbySelection[Nearby Selection] quick configuration.
