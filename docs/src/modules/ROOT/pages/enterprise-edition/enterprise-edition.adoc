...
Quarkus::
Automatic node sharing is enabled by default.
You can disable it by setting the property `quarkus.timefold.enterprise.solver.constraint-stream-automatic-node-sharing` to `false` in `application.properties`:
+
[source,properties,options="nowrap"]
----
quarkus.timefold.enterprise.solver.constraint-stream-automatic-node-sharing=false
----
======
