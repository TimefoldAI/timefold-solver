...
NOTE: Chained variable is deprecated and will be removed in Timefold Solver 2.0.
Use xref:using-timefold-solver/modeling-planning-problems.adoc#planningListVariable[list variable] instead.
...
[WARNING]
====
Models with xref:using-timefold-solver/modeling-planning-problems.adoc#chainedPlanningVariable[chained variables] cannot apply the partitioned search method.
====
...
xref:constraints-and-score/score-calculation.adoc#constraintStreams[Constraint streams] have builtin support for profiling constraints.
Profiling constraints allows you to identify which constraints are taking the majority of the time spent in score calculation and thus are worth optimizing.

NOTE: Traditional profiling tools such as Java Mission Control will report the internal classes used to calculate constraints instead of the constraints themselves.
...
When constraint stream profiling is enabled, a breakdown of time spent in each constraint is logged by the `ai.timefold.solver.enterprise.core.profiler.DefaultConstraintProfiler` class at the end of solving.

[source,text]
----
INFO: Constraint Profiling Summary
INFO: "Teacher time efficiency" (27.98%, 7.05 seconds)
INFO: "Teacher room stability" (13.99%, 3.07 seconds)
...
DEBUG: Constraint Profiling Breakdown
DEBUG: "Teacher time efficiency"
DEBUG:     FILTER (47.80%, 3.06 seconds)
TRACE:      By Operation
TRACE:        UPDATE (50.81%, 1.08 seconds)
TRACE:        RETRACT (31.00%, 1.01 seconds)
TRACE:        INSERT (18.19%, 0.07 seconds)
DEBUG:       By Node
DEBUG:         FILTER Node 5 defined at location org.acme.schooltimetabling.solver.TimetableConstraintProvider#teacherTimeEfficiency:79 (100.00%, 3.06 seconds)
...
----

IMPORTANT: xref:using-timefold-solver/running-the-solver.adoc#logging[Info logging must be configured and enabled] for the profiling report to be generated.

The following is reported:

* Info level: How much time spent in score calculation is spent by each constraint (relative percentage and total seconds) in descending order.
+
[source,text]
----
"Teacher time efficiency" (27.98%, 7.05 seconds)
----

* Debug level: A breakdown for each constraint (in descending order of time spent):

** Debug level: Time spent by each constraint stream building block (e.g., join, filter), shown as % and total seconds.
+
[source,text]
----
"Teacher time efficiency"
    FILTER (47.80%, 3.06 seconds)
----

** Each constraint stream building block is further broken down to:

*** Debug level: Time spent by each constraint stream building block, shown as % and total seconds of each each defined node and the location(s) the nodes were defined at.
+
[source,text]
----
    FILTER (47.80%, 3.06 seconds)
      ...
      By Node
        FILTER Node 5 defined at location org.acme.schooltimetabling.solver.TimetableConstraintProvider#teacherTimeEfficiency:79 (100.00%, 3.06 seconds)
----

*** Trace level: Time spent by each constraint stream building block, shown as % and total seconds of each propagation operation.
+
NOTE: This info is only relevant to the Timefold Solver team and can be safely ignored.
+
[source,text]
----
    FILTER (47.80%, 3.06 seconds)
      By Operation
        UPDATE (50.81%, 1.08 seconds)
        RETRACT (31.00%, 1.01 seconds)
        INSERT (18.19%, 0.07 seconds)
----


xref:enterprise-edition/enterprise-edition.adoc#nodeSharing[When node sharing] is used, the time spent will be reported with "Work shared by":

[source,text]
----
Work shared by "Room conflict", "Student group conflict", ... (11.10%, 2.10 seconds)
----
