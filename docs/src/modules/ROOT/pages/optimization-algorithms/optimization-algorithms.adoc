...
It does xref:constraints-and-score/constraints-and-score.adoc#incrementalScoreCalculation[incremental score calculation (deltas)] without any extra code.
...
Use this `Termination` where the perfect score is known, for example for four queens (which uses a xref:constraints-and-score/constraints-and-score.adoc#simpleScore[SimpleScore]):
...
A planning problem with a xref:constraints-and-score/constraints-and-score.adoc#hardSoftScore[HardSoftScore] may look like this:
...
A planning problem with a xref:constraints-and-score/constraints-and-score.adoc#bendableScore[BendableScore] with three hard levels and one soft level may look like this:
...
* Doing the move would break xref:constraints-and-score/constraints-and-score.adoc#buildInHardConstraint[a built-in hard constraint],
...
[IMPORTANT]
====
For the pruning to work with the default ``ScoreBounder``, the xref:constraints-and-score/constraints-and-score.adoc#initializingScoreTrend[InitializingScoreTrend] should be set.
Especially an xref:constraints-and-score/constraints-and-score.adoc#initializingScoreTrend[InitializingScoreTrend] of `ONLY_DOWN` (or at least has `ONLY_DOWN` in the leading score levels) prunes a lot.
====
...
This is the default if the xref:constraints-and-score/constraints-and-score.adoc#initializingScoreTrend[InitializingScoreTrend] is not ``ONLY_DOWN``.
+
[source,xml,options="nowrap"]
----
  <constructionHeuristic>
...
This is the default if the xref:constraints-and-score/constraints-and-score.adoc#initializingScoreTrend[InitializingScoreTrend] is ``ONLY_DOWN``.
+
[source,xml,options="nowrap"]
----
  <constructionHeuristic>
...
[NOTE]
====
If there are only negative constraints, but the xref:constraints-and-score/constraints-and-score.adoc#initializingScoreTrend[InitializingScoreTrend] is strictly not ``ONLY_DOWN``,
...
----
+
If the xref:constraints-and-score/constraints-and-score.adoc#initializingScoreTrend[InitializingScoreTrend] is ``ONLY_DOWN``, use `FIRST_FEASIBLE_SCORE_OR_NON_DETERIORATING_HARD` instead, because that's faster without any disadvantages.
...
If the xref:constraints-and-score/constraints-and-score.adoc#initializingScoreTrend[InitializingScoreTrend] is ``ONLY_DOWN``, a Construction Heuristic algorithm (such as First Fit) is faster:
...
* If an earlier move is better than a later move with the same score, the score calculator should add an extra softer xref:constraints-and-score/constraints-and-score.adoc#scoreLevel[score level] to score the first move as slightly better. Don't rely on move selection order to enforce that.
...
* *Multithreaded incremental solving*: solve 1 dataset with multiple threads without sacrificing xref:constraints-and-score/constraints-and-score.adoc#incrementalScoreCalculation[incremental score calculation].
...
and slow down the xref:constraints-and-score/constraints-and-score.adoc#scoreCalculationSpeed[score calculation speed].
====
...
and slows down the xref:constraints-and-score/constraints-and-score.adoc#scoreCalculationSpeed[score calculation speed].
====
