...
* ``DESCENDING``: Evaluate the planning values in descending order based on a given metric. Requires the model to support xref:running-timefold-solver/modeling-planning-problems.adoc#planningValueSorting[planning value sorting].
...
As shown in these time spent graphs from the xref:running-timefold-solver/benchmarking-and-tweaking.adoc#benchmarker[Benchmarker],
