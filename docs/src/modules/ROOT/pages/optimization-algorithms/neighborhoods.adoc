...
picking a single best match by a custom scoring rule, windowed/greedy pairing,
or combining three or more datasets by hand.
For those cases, `MoveStreamFactory` offers a parallel, lower-level API:
register datasets into opaque handles, then supply your own `Iterator<Move>` that combines them however you need.
...
public class BestShiftMoveProvider implements MoveProvider<Roster> {
    ...
    public MoveStream<Roster> build(MoveStreamFactory<Roster> factory) {
        var employees = factory.register(factory.forEach(Employee.class, false));
        var shifts = factory.register(factory.forEach(Shift.class, false));
...
`factory.register(...)` turns a `UniEnumeratingStream` or `BiEnumeratingStream` into a `UniDataset`/`BiDataset` handle,
mirroring `pick()` but without committing to the declarative approach.
Call `UniDataset#join(...)` to correlate two registered datasets using the same joiners as `pick()`;
unlike a joined dataset created via `pick()`, this join is not materialized upfront -
it is computed just in time, inside the resulting `BiDataset`.
