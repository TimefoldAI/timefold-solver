...
It further specializes into `PlanningVariableMetaModel` for a xref:running-timefold-solver/modeling-planning-problems.adoc#planningVariable[basic planning variable],
`PlanningListVariableMetaModel` for a xref:running-timefold-solver/modeling-planning-problems.adoc#planningListVariable[list variable],
and `ShadowVariableMetaModel` for a xref:running-timefold-solver/modeling-planning-problems.adoc#shadowPlanningVariables[shadow variable].
...
The following example shows a simple move implementation which changes the value of a xref:running-timefold-solver/modeling-planning-problems.adoc#planningVariable[basic planning variable],
...
`ChangeMove`:: Changes the value of a xref:running-timefold-solver/modeling-planning-problems.adoc#planningVariable[basic planning variable] on a planning entity.
`SwapMove`:: Swaps the values of a xref:running-timefold-solver/modeling-planning-problems.adoc#planningVariable[basic planning variable] between two planning entities.
`ListChangeMove`:: Changes the position of a value in the xref:running-timefold-solver/modeling-planning-problems.adoc#planningListVariable[list variable] of a planning entity.
`ListSwapMove`:: Swaps xref:running-timefold-solver/modeling-planning-problems.adoc#planningListVariable[list variable] elements between planning entities.
...
var moves = context.getMovesAsList(); // These are all moves the provider can generate for the given solution.

var move1 = moves.get(0);
assertThat(move1.getPlanningEntities())
    .containsExactly(...); // Assertions from a test framework of choice.
...
----
...
The API returns all moves that the move provider can generate for the given solution,
allowing you to write assertions to verify that the generated moves are correct.

The moves are retrieved in the order in which they are generated.
The exact ordering is unspecified, but it is deterministic
and consistent across multiple invocations with the same input.

If you expect your move provider to generate a large number of moves,
consider using the `getMovesAsStream()` or `getMovesAsIterator()` methods instead of `getMovesAsList()`,
to avoid loading all moves into memory at once,
or to filter them on-the-fly.
...
and can be combined with using `getMovesAsIterator()` to avoid loading all moves into memory at once:
