...
void testChangeMove() {
    ...
    var runner = MoveRunner.build(solutionMetaModel);
    var context = runner.using(timetable);
    context.execute(move);
...
[#neighborhoodsneighborhoodsMoveStreamsTesting]
=== Testing move generation with `NeighborhoodEvaluator`
...
The `NeighborhoodEvaluator` API provides a simple testing utility
...
IMPORTANT: The `NeighborhoodEvaluator` API is designed exclusively for testing purposes.
It should not be used in production code or during normal solver operation.
It is not thread-safe and not fine-tuned for performance.

[#neighborhoodsneighborhoodsMoveStreamsTestingBasics]
==== Basic usage

The `NeighborhoodEvaluator` API follows a fluent builder pattern:

[source,java,options="nowrap"]
----
// Timetable is the solution class, Lesson is a planning entity class.
var solutionMetaModel = PlanningSolutionMetaModel.of(Timetable.class, Lesson.class);
var evaluator = NeighborhoodEvaluator.build(new SwapMoveProvider(), solutionMetaModel);
...
[#neighborhoodsneighborhoodsMoveStreamsTestingRunningMoves]
==== Running the generated moves

The moves generated by the `NeighborhoodEvaluator` can be executed
using <<neighborhoodsMoveRunnerBasics,the MoveRunner API>>,
and an initialized `MoveRunContext` can be obtained directly from the `NeighborhoodEvaluatorContext` instance.

[source,java,options="nowrap"]
----
var evaluator = NeighborhoodEvaluator.build(new SwapMoveProvider(), solutionMetaModel);
var evaluatorContext = evaluator.using(solution);
var moveRunContext = evaluatorContext.getMoveRunContext();
----
...
var evaluator = NeighborhoodEvaluator.build(new SwapMoveProvider(), solutionMetaModel);
var evaluatorContext = evaluator.using(solution);
var moveIterator = evaluatorContext.getMovesAsIterator();

assertThat(moveIterator).hasNext();
var firstMove = moveIterator.next();

var moveRunContext = evaluatorContext.getMoveRunContext();
moveRunContext.executeTemporarily(firstMove, solutionView -> assertThat(firstEntity.getValue())
...
you must re-obtain the move iterator from the `NeighborhoodEvaluatorContext` instance.
