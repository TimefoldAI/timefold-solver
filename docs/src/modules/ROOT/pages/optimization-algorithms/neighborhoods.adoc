    ...
    public MoveStream<Roster> build(MoveStreamFactory<Roster> factory) {
        ...
        return factory.buildMoveStream((session, random) -> {
            ...
            return new Iterator<Move<Roster>>() {
                private final Iterator<Employee> employeeIterator = employeeInstance.randomIterator(random);
...
which exposes `size()`, `iterator()`, `randomIterator(RandomGenerator)` and `uniqueRandomIterator(RandomGenerator)` -
 the same random-walk sampling used throughout Move Streams.
 A `BiDatasetInstance` additionally exposes the same four operations keyed by a left-side value,
...
Because the order in which a move provider generates moves is never part of the Move Streams contract,
