...
This is only necessary when the solver is configured to use xref:using-timefold-solver/running-the-solver.adoc#multithreadedSolving[multi-threaded solving].
`String describe()`::
...
The same pattern applies to other building blocks as well, such as `join`, `ifExists` or `groupBy`;
