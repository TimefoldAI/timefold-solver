...
Because `MoveRunner` implements `AutoCloseable`, you must use it within a try-with-resources block
to ensure proper cleanup of internal solver resources.
...
but you typically won't need to use it—you can directly inspect the solution's state
...
If a move may throw exceptions during execution, you can provide an exception handler:
...
void testMoveWithExceptionHandler() {
    ...
    var move = Moves.change(variableMetaModel, entity, value);
    var exceptions = new ArrayList<Exception>();

    try (var runner = MoveRunner.build(SolutionClass.class, EntityClass.class)) {
        var context = runner.using(solution);
        context.execute(move, exceptions::add);
    }
