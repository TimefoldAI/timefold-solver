...
xref:running-timefold-solver/solver-diagnostics.adoc#logging[Turn on `trace` logging] to show the decision making in the log.
...
* Random tie breaking does not affect xref:running-timefold-solver/solver-diagnostics.adoc#environmentMode[reproducibility].
====
...
xref:running-timefold-solver/solver-diagnostics.adoc#logging[Turn on INFO logging] to see the step times.
