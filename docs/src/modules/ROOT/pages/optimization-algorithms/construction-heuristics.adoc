...
Requires the model
to support xref:design-patterns/modeling-planning-problems.adoc#planningEntitySorting[planning entity sorting].
...
Requires the model
to support xref:design-patterns/modeling-planning-problems.adoc#planningValueSorting[planning value sorting].
...
Requires the model
to support xref:design-patterns/modeling-planning-problems.adoc#planningEntitySorting[planning entity sorting]
and xref:design-patterns/modeling-planning-problems.adoc#planningValueSorting[planning value sorting].
...
Requires the model
to support xref:design-patterns/modeling-planning-problems.adoc#planningEntitySorting[planning entity sorting].
* `DESCENDING_IF_AVAILABLE` (default): If the model supports xref:design-patterns/modeling-planning-problems.adoc#planningEntitySorting[planning entity sorting], behave like ``DESCENDING``, else like ``NONE``.
...
Requires the model
to support xref:design-patterns/modeling-planning-problems.adoc#planningValueSorting[planning value sorting].
* `ASCENDING_IF_AVAILABLE` (default): If the model supports xref:design-patterns/modeling-planning-problems.adoc#planningValueSorting[planning value sorting], behave like ``ASCENDING``, else like ``NONE``.
* ``DESCENDING``: Evaluate the planning values in descending order based on a given metric.
Requires the model to support xref:design-patterns/modeling-planning-problems.adoc#planningValueSorting[planning value sorting].
* ``DESCENDING_IF_AVAILABLE``: If the model supports xref:design-patterns/modeling-planning-problems.adoc#planningValueSorting[planning value sorting], behave like ``DESCENDING``, else like ``NONE``.
...
The xref:optimization-algorithms/move-selector-reference.adoc#mimicSelection[mimic selection] ensures that the winning `Move` changes only the selected entity.

To customize the entity or value sorting,
see xref:optimization-algorithms/move-selector-reference.adoc#sortedSelection[sorted selection].
For scaling out, see <<scalingConstructionHeuristics,scaling construction heuristics>>.
...
Use the xref:running-timefold-solver/benchmarking-and-tweaking.adoc#benchmarker[Benchmarker]
...
(such as xref:optimization-algorithms/move-selector-reference.adoc#filteredSelection[filtering]
and xref:optimization-algorithms/move-selector-reference.adoc#limitedSelection[limiting])
is supported too.
...
The default behavior of the construction heuristic for xref:design-patterns/modeling-planning-problems.adoc#mixedModels[mixed models] is
...
and less than 50 milliseconds in xref:design-patterns/responding-to-change.adoc#realTimePlanning[real-time planning],
so there is plenty of time left for xref:optimization-algorithms/local-search.adoc#localSearch[Local Search].
If the xref:running-timefold-solver/benchmarking-and-tweaking.adoc#benchmarker[Benchmarker] proves that this is not the case, there's a number of improvements that can be done:
...
Let the xref:running-timefold-solver/benchmarking-and-tweaking.adoc#benchmarker[Benchmarker] guide you.
====
...
xref:running-timefold-solver/multithreaded-solving.adoc#partitionedSearch[Partitioned Search] reduces the number of moves per step.
...
* xref:optimization-algorithms/move-selector-reference.adoc#filteredSelection[Filtered selection]
* xref:optimization-algorithms/move-selector-reference.adoc#limitedSelection[Limited selection]
