...
. Next,
implement xref:domain-modeling/modeling-planning-problems.adoc#planningEntitySorting[planning entity sorting]
...
Configure xref:running-timefold-solver/solver-diagnostics.adoc#logging[logging] to display the log messages of each scope.
...
This is especially common during xref:responding-to-change/real-time-planning.adoc[real-time planning].
====
...
Switching xref:running-timefold-solver/solver-diagnostics.adoc#environmentMode[EnvironmentMode] can heavily impact when this termination ends.
...
NOTE: Most of the time, a custom solver phase is not worth the investment of development time.
...
or build one using the xref:optimization-algorithms/neighborhoods.adoc#neighborhoodsBuiltInMoves[built-in `Moves` factory].
...
which may have been xref:domain-modeling/modeling-planning-problems.adoc#cloningASolution[planning-cloned] since.
...
IMPORTANT: Do not change any of the problem facts in a `PhaseCommand` or add or remove entities,
it will corrupt the `Solver` because any previous score or solution was for a different problem.
To change problem facts or to add or remove entities,
read about xref:responding-to-change/real-time-planning.adoc[repeated planning]
and use xref:responding-to-change/real-time-planning.adoc#problemChange[ProblemChange] instead.
...
NOTE: If the changes performed in a `PhaseCommand` do not result in a better score,
...
add the `customProperties` element and use xref:running-timefold-solver/library/configuration.adoc#customPropertiesConfiguration[custom properties]:
...
run a xref:running-timefold-solver/benchmarking-and-tweaking.adoc#benchmarker[Benchmarker] __for a short amount of time__
and xref:running-timefold-solver/benchmarking-and-tweaking.adoc#writeTheOutputSolutionOfBenchmarkRuns[configure it to write the best solutions to disk].
