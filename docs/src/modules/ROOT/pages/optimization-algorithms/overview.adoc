...
===== Sorted selection by `SorterFactory`

If you need the entire solution to sort a ``Selector``, use a `SorterFactory` instead:
...
[NOTE]
====
`SorterFactory` implementations are expected to be stateless.
The solver may choose to reuse them in different contexts.
====
