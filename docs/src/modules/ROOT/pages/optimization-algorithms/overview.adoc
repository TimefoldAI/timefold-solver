...
===== Sorted selection by `SorterWeightFactory`

If you need the entire solution to sort a ``Selector``, use a `SorterWeightFactory` instead:
...
[NOTE]
====
`SorterWeightFactory` implementations are expected to be stateless.
The solver may choose to reuse them in different contexts.
====
