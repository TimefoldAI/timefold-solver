...
Especially for xref:constraints-and-score/constraints-and-score.adoc#constraintStreams[Constraint Streams],
...
Even if we optimize the code above to use ``Map``s to iterate through the `processList` only once, _it is still slow_ because it does not do xref:constraints-and-score/constraints-and-score.adoc#incrementalScoreCalculation[incremental score calculation].
