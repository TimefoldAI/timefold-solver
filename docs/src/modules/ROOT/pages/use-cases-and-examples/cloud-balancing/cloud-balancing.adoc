...
[NOTE]
====
Instead of getter annotations, it is also possible to use xref:using-timefold-solver/configuration.adoc#annotationAlternatives[field annotations].
====
...
which can come from xref:using-timefold-solver/configuration.adoc#solverConfigurationByXML[an XML file] as classpath resource:
+
[source,java,options="nowrap"]
----
        SolverFactory<CloudBalance> solverFactory = SolverFactory.createFromXmlResource(
...
----
+
Or to avoid XML, build it through xref:using-timefold-solver/configuration.adoc#solverConfigurationByJavaAPI[the programmatic API] instead:
+
[source,java,options="nowrap"]
----
...
Use the xref:using-timefold-solver/benchmarking-and-tweaking.adoc#benchmarker[Benchmarker] to xref:optimization-algorithms/optimization-algorithms.adoc#powerTweaking[power tweak] to get even better results.
