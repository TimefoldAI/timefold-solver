...
public class Job {
    ...
    @ShadowSources("startDate")
    public LocalDate endDateSupplier(PackagingSchedule schedule) {
        if (startDate == null) {
            return null;
        } else {
            return startDate.plusDays(durationInDays);
...
[NOTE]
====
Note that the supplier method `endDateSupplier` has an optional parameter,
which is the planning solution class `PackagingSchedule`.
No other parameters are accepted except for the solution class.
====
