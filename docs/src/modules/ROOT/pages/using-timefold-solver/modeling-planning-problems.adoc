...
==== Grouping Shadow Variables

`@ShadowSources` has an optional `groupKey` parameter that can be used to reuse calculation results for related entities.
To be used, a few requirements must be met:

* `groupKey` must be the name of a problem fact on the entity; its value cannot change during solving.

* For all entities with the same, non-null `groupKey` value, calling the supplier method will result in the same value regardless of what entity is called.
...
Meets the requirements since the `leader` field is not a variable, and entities with the same `leader` will get the same value.
...
Does not meet the requirement, since `Follower` entities with the same `leader` but a different `minValue` may calculate different values.

Entities where the `groupKey` is evaluated to `null` are each calculated separately and won't share calculations for that shadow variable with other entities.
...
public class Job {
    ...
    @ShadowSources(value = {"readyDate", "alignedJobs[].readyDate"}, groupKey="alignedJobs")
    public LocalDate startDateSupplier() {
        if (alignedJobs == null) {
            // All entities with a null groupKey (in this case, "alignedJobs")
