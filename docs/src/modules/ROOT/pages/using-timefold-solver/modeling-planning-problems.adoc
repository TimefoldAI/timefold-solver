...
For example in school timetabling, the timeslots and rooms are problem facts:
...
For example in school timetabling, a `Lesson` is defined by its subject, teacher and a student group,
...
For example, a ``Lessons``'s `timeslot` variable should not be used.
...
For example, a ``Lessons``'s `timeslot` property is a genuine planning variable.
Note that even though a ``Lessons``'s `timeslot` property changes to another `Timeslot` during planning,
...
For example in school timetabling, that `Timetable` class has the `@PlanningSolution` annotation,
...
Create a `@PlanningSolution` instance to represent your planning problem's dataset,
so it can be set on the `Solver` as the planning problem to solve.
For example in school timetabling, a `Lesson` instance is created with the required `Lesson` instances,
but their timeslots and rooms are set to `null`.
Usually, most of this data comes from your data layer,
and your solution implementation just aggregates that data
and creates the uninitialized planning entity instances for the solver to plan