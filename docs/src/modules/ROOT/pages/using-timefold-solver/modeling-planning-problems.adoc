...
Moreover,
it automatically propagates changes to the next elements when the value of the related shadow variable changes.
...
the use of `@CascadingUpdateShadowVariable` as a source for other variables is not allowed.
====

==== Multiple sources

If the user-defined logic requires updating multiple shadow variables,
apply the `@CascadingUpdateShadowVariable` annotation to only one shadow variable.
Next,
use the `@PiggybackShadowVariable` for all other shadow variables
that will also be updated by the specified target method.
...
[WARNING]
====
It is not allowed to specify the property `shadowEntityClass` in `@PiggybackShadowVariable`
