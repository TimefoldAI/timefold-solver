...
on how inheritance can be used when it comes defining planning solution models:
...
The following example illustrates two planning solutions, where the child class `TestdataExtendedSolution` inherits from `TestdataBaseSolution`. Additionally, multiple annotated members, `valueList`, `entityList`, `score`, and `secondEntityList`, are accessed.
...
----
@PlanningSolution
public class TestdataBaseSolution {
    @ProblemFactCollectionProperty
...
@PlanningSolution
public class TestdataExtendedSolution extends TestdataBaseSolution {
    @PlanningEntityCollectionProperty
...
we can see that the class `TestdataExtendedSolution` does not have the `@PlanningSolution` annotation.
...
The model `TestdataBaseSolution` includes an annotated member but lacks the `@PlanningSolution` annotation.
...
@PlanningSolution
public class TestdataChildSolution extends TestdataBaseSolution {
    ...
}


@PlanningSolution
public class TestdataExtendedSolution extends TestdataChildSolution {
    ...
}
----
====
...
where the child class `TestdataChildEntity` inherits from `TestdataBaseEntity`.
...
In the next code snippet, it can be observed that `TestdataChildEntity` does not have the `@PlanningEntity` annotation,
but it is accepted as an entity because its direct parent is an annotated entity.
Additionally, the planning variable `value` will be linked with `TestdataChildEntity`.
...
The model `TestdataBaseEntity` defines a planning variable.
...
@PlanningEntity
public class TestdataMultipleChildEntity extends TestdataChildEntity
    implements TestdataBaseEntity {
    ...
}

@PlanningEntity
public class TestdataChildEntity {
    ...
}
