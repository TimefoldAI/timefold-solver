...
To define a declarative shadow variable: Annotate the field with `@ShadowVariable` and specifying the name of a supplier method.
...
|"variableName"
|Refers to a variable (planning or shadow) on the same planning entity.
...
For example, if `c` depends on `a`, and `a` is in a loop with `b`, then `c` is also looped.
...
This property (isLooped in the example above) is typically used in a constraint filter to penalize looped entities via a hard constraint,
...
on how inheritance can be used when it comes to planning entities and planning variables:
