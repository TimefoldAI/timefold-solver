...
Variables that are different between from scratch and before:
  - Actual value (oldValue) of variable value on EntityClass entity (entity) differs from expected (newValue)
```
...
The TRACED_FULL_ASSERT mode is by far the slowest mode (because it clones solutions before and after each move).
