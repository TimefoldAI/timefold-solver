...
A `SolverJobBuilder` includes various settings, as shown in the following code snippet:
...
while the problem is defined with `withProblem(problem)`.
...
Finally, some solver settings, such as the termination configuration, can be overridden using `withConfigOverride(...)`.

[WARNING]
====
The solutions returned by the events in `withFirstInitializedSolutionConsumer(...)`
and `withBestSolutionConsumer(...)` can be reused during the solving process.
Therefore, do not apply any changes to the solution instance while the solver is running,
as any modifications may lead to solver corruption.
====
