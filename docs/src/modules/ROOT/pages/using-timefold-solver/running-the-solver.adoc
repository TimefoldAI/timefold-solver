...
Typically, the problem involves uninitialized planning variables that the solver will optimize.
...
Additionally,
an improved solution consumer capable of throttling events is available in the xref:enterprise-edition/enterprise-edition.adoc#throttlingBestSolutionEvents[enterprise product].
...
[NOTE]
====
The `SolverJobBuilder` contract includes many settings, but only two are required: `withProblemId(...)` and `withProblem(...)`.
====
