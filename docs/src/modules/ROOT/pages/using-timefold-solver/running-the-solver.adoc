...
The `PHASE_ASSERT` mode can be slightly slower than the `NON_REPRODUCIBLE` mode.
...
The `PHASE_ASSERT` mode is negligibly slower than the `NO_ASSERT` mode,
but it gives you the benefit of quickly checking for score corruptions.
