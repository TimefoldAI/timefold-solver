...
without losing reproducibility (for each sub single index) in xref:using-timefold-solver/running-the-solver.adoc#environmentMode[EnvironmentMode] ``NO_ASSERT`` and lower.
====
...
For automated or integrated workflows, you can use the `BenchmarkAggregator.aggregate()` method directly to create aggregated reports programmatically. This approach is particularly useful in CI/CD pipelines, data science analytics, research studies, or when you need a comprehensive view of multiple benchmark runs as part of a larger application workflow.

[source,java,options="nowrap"]
----
public File aggregateBenchmarks() {
    ...
    return aggregator.aggregate(allSingleBenchmarkResults, solverBenchmarkResultNameMap);
}
----

The `BenchmarkAggregator.aggregate()` method takes two parameters:

* `allSingleBenchmarkResults`: A list of **SingleBenchmarkResult** objects from all the benchmarks you want to aggregate
* `solverBenchmarkResultNameMap`: A map that preserves the original solver names of **SolverBenchmarkResult** for proper identification in the aggregated report

The method returns a `File` pointing to the generated HTML overview report that contains the aggregated benchmark results.
