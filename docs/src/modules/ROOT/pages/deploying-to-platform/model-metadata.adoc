...
Once your model is xref:deploying-to-platform/guide.adoc[deployed to Timefold Platform], the platform UI surfaces several pieces of your model's code and configuration directly.
...
Every model ships with a default configuration profile, defined through `timefold.model.default-config.*` properties in `application.properties`.
