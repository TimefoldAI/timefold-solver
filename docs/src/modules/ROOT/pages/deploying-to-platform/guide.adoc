= Deploying to the platform guide (Preview)
...
Learn more about xref:deploying-to-platform/introduction.adoc[Deploying to the Timefold Platform].
...
A finished version of this service is available in the https://github.com/TimefoldAI/timefold-quickstarts/tree/stable/getting-started/service[`getting-started/service` quickstart].
...
* A container runtime (like Docker) installed and running locally. During deployment, a container image is built, which requires one even if you don't interact with it directly.
...
If you forget, the `timefold:configure` goal fails the build during the `initialize` phase, before the container image is built and pushed.
It checks that an `ai.timefold.solver.enterprise` artifact is actually on the classpath, and that your model inherits from `timefold-solver-service-parent`.

Building with the Enterprise Edition does not itself require a license; a license is only checked when the model actually runs.
However, if your build runs tests that exercise Enterprise Edition features, those tests run the solver, so the build fails without a valid license.
See xref:commercial-editions/license-management.adoc[License management] to register for a free trial license.
...
You may still want a license if you want to run your Enterprise Edition build locally, for example to test the model with `mvn quarkus:dev` before deploying it.
...
When selecting its permissions, scope the token to the tenant you want to register the model in, and grant *Create*, *Read*, *Update*, and *Delete* on the *Registered Model* resource, since the plugin needs all 4 to register, update, and later undeploy your model.
...
=== Preview without pushing the container
