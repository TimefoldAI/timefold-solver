...
[source,bash,options="nowrap"]
----
mvn clean package timefold:deploy
----
...
Run `mvn clean package -Dupdate-api` once first to generate it, as described in xref:running-timefold-solver/service/rest-api.adoc#deliberateAPIChanges[Deliberate API changes], then retry the command above.
====
