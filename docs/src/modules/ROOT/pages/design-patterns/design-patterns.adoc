...
. *If there are multiple relationships (or fields), check for xref:configuration/configuration.adoc#shadowVariable[shadow variables]*.
...
The other side will become an xref:configuration/configuration.adoc#bidirectionalVariable[inverse relation shadow variable] later on. Keep bi-directional relationships orange.
====
...
[NOTE]
====
Course scheduling is different because it uses two xref:configuration/configuration.adoc#planningVariable[planning variables].
Vehicle routing is different because it uses a xref:configuration/configuration.adoc#planningListVariable[planning list variable].
Traveling salesman is different because it uses a xref:configuration/configuration.adoc#chainedPlanningVariable[chained planning variable].
====
...
use a xref:configuration/configuration.adoc#shadowVariable[shadow variable] to calculate the time.

This pattern is implemented using either the xref:configuration/configuration.adoc#chainedPlanningVariable[chained planning variable] or the xref:configuration/configuration.adoc#planningListVariable[planning list variable].
...
Using the xref:configuration/configuration.adoc#chainedPlanningVariable[chained planning variable], planning entities are arranged in a recursive data structure, forming a chain, that ends with an anchor.
...
The starting time of an entity is usually a xref:configuration/configuration.adoc#shadowVariable[shadow variable].
...
Timefold Solver distributes planning values into planning entities’ xref:configuration/configuration.adoc#planningListVariable[planning list variable].
...
It lasts 52 minutes, therefore her second task starts at 8:52.
The starting time of an element is usually a xref:configuration/configuration.adoc#shadowVariable[shadow variable].
...
Implement the automatic collapse in the xref:configuration/configuration.adoc#customVariableListener[custom variable listener]
...
Implement the automatic delay in the xref:configuration/configuration.adoc#customVariableListener[custom variable listener]
