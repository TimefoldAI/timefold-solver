...
Timefold design patterns are generic reusable solutions to common challenges in the model or architecture of projects that perform constraint solving. The design patterns in this section list and solve common design challenges.
...
because Timefold cannot assign an extra skill to an employee.
...
[NOTE]
====
Timefold does not currently support a `@PlanningVariable` annotation on a collection.
====
...
. *Choose the model in which the number of planning entities is fixed during planning*. For example, in the employee rostering, it is impossible to know in advance how many shifts each employee will have before Timefold solves the model and the results can differ for each solution found.
...
The two modeling approaches are equivalent because they both allow Timefold to order elements in sequences of variable lengths.
...
Timefold distributes planning values into planning entities’ xref:configuration/configuration.adoc#planningListVariable[planning list variable].
...
There are two common usage patterns of Timefold in the cloud:
