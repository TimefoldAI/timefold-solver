...
:page-aliases: repeated-planning/repeated-planning.adoc,responding-to-change/responding-to-change.adoc
:doctype: book
:sectnums:
:icons: font
:todo: https://github.com/TimefoldAI/timefold-solver/issues/2257
...
* Use xref:design-patterns/modeling-planning-problems.adoc#planningVariableAllowingUnassigned[planning variables with unassigned values],
so that some entities are unassigned.
* Add virtual values to catch the unassigned entities. (Not recommended)
...
. Make the planning variable xref:design-patterns/modeling-planning-problems.adoc#planningVariableAllowingUnassigned[allow unassigned values].
...
Use xref:constraints-and-score/score-calculation.adoc#constraintStreamsForEach[`forEachIncludingUnassigned`] and check if the xref:design-patterns/modeling-planning-problems.adoc#planningVariable[planning variable] or xref:design-patterns/modeling-planning-problems.adoc#bidirectionalVariable[inverse relation shadow variable] is `null`:
...
They contain only xref:design-patterns/modeling-planning-problems.adoc#pinnedPlanningEntities[pinned] and/or xref:how-to/nonvolatile-replanning.adoc#nonvolatileReplanning[semi-movable] planning entities.
+
...
+
Changing that schedule later is disruptive, but were exceptions force us to do them anyway (for example someone calls in sick), do change this part of the planning while minimizing disruption with xref:how-to/nonvolatile-replanning.adoc#nonvolatileReplanning[non-disruptive replanning].

* _Draft_
+
Upcoming time periods after the published time periods that can change freely.
They contain movable planning entities, except for any that are pinned for other reasons (such as being xref:design-patterns/modeling-planning-problems.adoc#pinDownPlanningEntities[pinned by a user]).
+
...
** If xref:design-patterns/time-patterns.adoc#assigningTimeToPlanningEntities[time is a planning variable], the size of the planning window is determined dynamically,
...
Timefold Solver can handle such scenarios with `ProblemChange` (in combination with xref:design-patterns/modeling-planning-problems.adoc#pinnedPlanningEntities[pinned planning entities]).
...
it is important to understand the behavior of xref:design-patterns/modeling-planning-problems.adoc#cloningASolution[a planning clone].
...
. Any change in a `ProblemChange` must be done on the `@PlanningSolution` instance provided to the `ProblemChange` implementation.

. The `workingSolution` is xref:design-patterns/modeling-planning-problems.adoc#cloningASolution[a planning clone] of the ``BestSolutionChangedEvent``'s ``bestSolution``.
...
This requires xref:design-patterns/modeling-planning-problems.adoc#planningId[annotating a property of that class as the @PlanningId].
...
(unless you have a xref:design-patterns/modeling-planning-problems.adoc#planningVariableAllowingUnassigned[planning variable with unassigned values]),
...
Similarly to xref:running-timefold-solver/multithreaded-solving.adoc#partitionedSearch[Partitioned Search], multi-stage planning leads to suboptimal results.
