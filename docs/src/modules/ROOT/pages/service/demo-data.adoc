...
public class TimetableDemoDataGenerator implements DemoDataGenerator {
    ...
    public enum DemoDataKind {
        ...
        private final Function<DemoDataKind, ModelRequest<?>> requestFunction;

        DemoDataKind(DemoMetaData metaData, Function<DemoDataKind, ModelRequest<?>> requestFunction) {
...
- `GET /&lt;root&gt;/demo-data`: Retrieve all available demo datasets.
- `GET /&lt;root&gt;/demo-data/\{id\}`: Retrieve demo dataset with given identifier
