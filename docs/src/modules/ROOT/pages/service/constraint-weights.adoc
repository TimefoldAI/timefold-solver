...
Constraint weights are always an interpretation by the modeller. It might be that the consumer of the model would like to see the constraints weighed differently.
...
// TODO, i wish it ended with The SDK takes care of changing the referenced constraint with the new weight... but we continue!
...
More information about https://docs.timefold.ai/timefold-solver/latest/constraints-and-score/constraint-configuration[Adjusting constraints at runtime] in the Timefold Solver documentation.

//TODO === Adjusting Model Parameters