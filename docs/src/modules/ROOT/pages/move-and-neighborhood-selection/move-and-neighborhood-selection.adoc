...
[NOTE]
====
Nearby selection is a commercial feature of Timefold Enterprise.
...
Timefold calls `AbstractMove.doMove(ScoreDirector)`, which calls `doMoveOnGenuineVariables(ScoreDirector)`.
...
Timefold automatically filters out _non doable moves_ by calling the `isMoveDoable(ScoreDirector)` method on each selected move.
...
Always implement the `toString()` method to keep Timefold's logs readable.
