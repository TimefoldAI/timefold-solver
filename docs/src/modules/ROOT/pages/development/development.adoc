...
The diagram below explains the overall structure of the Timefold Solver source code:
...
. *Fail Fast at runtime in assertion mode* if the detection performance cost is high. For example: If, after every low level iteration, the variable A needs to be equal to the square root of B, check it if and only if an assert flag is set to true (usually controlled by the xref:configuration/configuration.adoc#environmentMode[EnvironmentMode]).
