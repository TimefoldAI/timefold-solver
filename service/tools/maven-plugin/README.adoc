...
- Fetches the token identity by calling GET /api/platform/v1/aboutme and prints a human-readable summary: user, granted scopes (permissions), tenants, namespaces, the selected tenant, and whether the token is allowed to register/update models
...
- The platform validates the model version (`timefold.model.api-version`) on registration. It must be a release version `vN` (e.g. `v1`), a git commit hash (1–40 lowercase hex characters), or the literal `SNAPSHOT`; any other value is rejected with HTTP 400.
