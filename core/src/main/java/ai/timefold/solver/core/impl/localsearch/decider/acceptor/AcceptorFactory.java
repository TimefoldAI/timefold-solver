...
public class AcceptorFactory<Solution_> {
    ...
    private HillClimbingAcceptor<Solution_> buildHillClimbingAcceptor() {
    ...
    private StepCountingHillClimbingAcceptor<Solution_> buildStepCountingHillClimbingAcceptor() {
    ...
    private EntityTabuAcceptor<Solution_> buildEntityTabuAcceptor(HeuristicConfigPolicy<Solution_> configPolicy) {
        ...
        if (!acceptorTypeListsContainsAcceptorType(AcceptorType.ENTITY_TABU) && entityTabuSize == null
                && entityTabuRatio == null && fadingEntityTabuSize == null && fadingEntityTabuRatio == null) {
            return null;
        }
        ...
        acceptor.enableAssertions(configPolicy.getEnvironmentMode());
        return acceptor;
    }

    private ValueTabuAcceptor<Solution_> buildValueTabuAcceptor(HeuristicConfigPolicy<Solution_> configPolicy) {
        var valueTabuSize = acceptorConfig.getValueTabuSize();
        var fadingValueTabuSize = acceptorConfig.getFadingValueTabuSize();
        if (!acceptorTypeListsContainsAcceptorType(AcceptorType.VALUE_TABU) && valueTabuSize == null
        ...
        configureFixedSizeTabuAcceptor(acceptor, configPolicy.getEnvironmentMode(), valueTabuSize, fadingValueTabuSize);
        return acceptor;
    }
    ...
    private MoveTabuAcceptor<Solution_> buildMoveTabuAcceptor(HeuristicConfigPolicy<Solution_> configPolicy) {
        var moveTabuSize = acceptorConfig.getMoveTabuSize();
        var fadingMoveTabuSize = acceptorConfig.getFadingMoveTabuSize();
        if (!acceptorTypeListsContainsAcceptorType(AcceptorType.MOVE_TABU) && moveTabuSize == null
        ...
        configureFixedSizeTabuAcceptor(acceptor, configPolicy.getEnvironmentMode(), moveTabuSize, fadingMoveTabuSize);
        return acceptor;
    }

    private SimulatedAnnealingAcceptor<Solution_>
    ...
    private LateAcceptanceAcceptor<Solution_> buildLateAcceptanceAcceptor() {
        var hasLA = acceptorTypeListsContainsAcceptorType(AcceptorType.LATE_ACCEPTANCE);
        var hasDLAS = acceptorTypeListsContainsAcceptorType(AcceptorType.DIVERSIFIED_LATE_ACCEPTANCE);
        var hasSetting = acceptorConfig.getLateAcceptanceSize() != null;
        if ((!hasLA && !hasSetting) || (hasSetting && !hasLA && hasDLAS)) {
            return null;
        }
    ...
    private DiversifiedLateAcceptanceAcceptor<Solution_>
    ...
    private GreatDelugeAcceptor<Solution_> buildGreatDelugeAcceptor(HeuristicConfigPolicy<Solution_> configPolicy) {
