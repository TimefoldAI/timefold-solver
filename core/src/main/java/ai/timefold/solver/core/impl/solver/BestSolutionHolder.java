...
final class BestSolutionHolder<Solution_> {
    ...
    private synchronized SortedMap<BigInteger, List<CompletableFuture<Void>>> replaceMapSynchronized(
            Function<SortedMap<BigInteger, List<CompletableFuture<Void>>>, SortedMap<BigInteger, List<CompletableFuture<Void>>>> newMap) {
        var oldMap = problemChangesPerVersionMap;
        problemChangesPerVersionMap = newMap.apply(oldMap);
        return oldMap;
    }
