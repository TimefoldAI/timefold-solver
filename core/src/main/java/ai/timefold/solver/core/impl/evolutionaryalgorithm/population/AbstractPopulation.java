...
/**
 * Base class for population implementations of the evolutionary algorithm.
 * <p>
 * Manages the population of candidate solutions following the biased-fitness survival strategy
 * described in the HGS (Hybrid Genetic Search) original article.
 * <p>
 ...
 */
@NullMarked
public abstract class AbstractPopulation<Solution_, Score_ extends Score<Score_>> implements Population<Solution_, Score_> {

    protected static final Logger LOGGER = LoggerFactory.getLogger(AbstractPopulation.class);

    protected final int populationSize;
    protected final int eliteSolutionSize;
    protected final int maxSize;
    protected final List<InternalIndividual<Solution_, Score_>> feasibleIndividualList;
    protected final List<InternalIndividual<Solution_, Score_>> infeasibleIndividualList;
    protected final PopulationDiffMap<Solution_, Score_> diffMap;
    @Nullable
    protected Individual<Solution_, Score_> bestIndividual = null;
    protected long bestGeneration = 0L;
    protected long bestIteration = 0L;
    protected long generationCount = 0L;
    protected long individualCount = 0L;

    protected boolean feasibleFitnessUpdated = false;
    protected boolean infeasibleFitnessUpdated = false;

    protected AbstractPopulation(int populationSize, int generationSize, int eliteSolutionSize) {
    ...
    private InternalIndividual<Solution_, Score_> addIndividualToList(Individual<Solution_, Score_> individual) {
        ...
        var newBestSolution = bestIndividual == null || internalIndividual.compareTo(bestIndividual) > 0;
        if (newBestSolution) {
        ...
        if (LOGGER.isDebugEnabled()) {
            LOGGER.debug(
                    "Added individual iteration ({}), generation({}), feasible population size({}), infeasible population size({}), individual score ({}), first parent score ({}), second parent score ({}), best score ({}), best generation ({}), best iteration ({}), **new best solution({})",
                    ...
                    bestIndividual.getScore().raw(), bestGeneration, bestIteration, newBestSolution);
        }
    ...
    @Override
    public Individual<Solution_, Score_> selectIndividual(RandomGenerator workingRandom) {
    ...
    @Nullable
    @Override
    public Individual<Solution_, Score_> getBestIndividual() {
        return bestIndividual;
    }

    protected static class InternalIndividual<Solution_, Score_ extends Score<Score_>>
            implements Individual<Solution_, Score_> {

        final Individual<Solution_, Score_> innerIndividual;
        private double fitness;

        protected InternalIndividual(Individual<Solution_, Score_> innerIndividual) {
