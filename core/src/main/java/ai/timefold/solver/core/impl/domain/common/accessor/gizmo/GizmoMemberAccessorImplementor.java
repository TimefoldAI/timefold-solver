...
public final class GizmoMemberAccessorImplementor {
     ...
     * @param requiredReturnType A flag that indicates if the return type is required or optional.
    ...
    static MemberAccessor createAccessorFor(Member member, Class<? extends Annotation> annotationClass,
        ...
        GizmoMemberInfo memberInfo =
                new GizmoMemberInfo(new GizmoMemberDescriptor(member), requiredReturnType, annotationClass);
    ...
    private static void createConstructor(ClassCreator classCreator, GizmoMemberInfo memberInfo) {
        ...
        memberInfo.descriptor().whenMetadataIsOnMethod(md -> {
            ...
            if (memberInfo.isRequiredReturnType()) {
                // We create a field to store the result, only if the called method has a return type; otherwise, we
    ...
    private static void assertIsGoodMethod(MethodDescriptor method, boolean requiredReturnType,
        ...
        if (methodName.startsWith("get")) {
            ...
            // must be a read method and return a result only if requiredReturnType is true
            if (requiredReturnType && method.getReturnType().equals("V")) {
    ...
    private static void createGetName(ClassCreator classCreator, GizmoMemberInfo memberInfo) {
        ...
        memberInfo.descriptor().whenIsMethod(method -> {
            ...
            if (annotationClass == null) {
                assertIsGoodMethod(method, memberInfo.isRequiredReturnType());
            } else {
                assertIsGoodMethod(method, memberInfo.isRequiredReturnType(), annotationClass);
            }
        });
