...
class SelectorBasedListChangeMoveTest {
    ...
    void isMoveDoable() {
        ...
        assertThat(new SelectorBasedListChangeMove<>(variableDescriptor, e1, 1, e1, 1).isMoveDoable(scoreDirector)).isFalse();
        // same entity, different index => doable
        assertThat(new SelectorBasedListChangeMove<>(variableDescriptor, e1, 0, e1, 1).isMoveDoable(scoreDirector)).isTrue();
        // same entity, index == list size => not doable because the element is first removed (list size is reduced by 1)
        assertThat(new SelectorBasedListChangeMove<>(variableDescriptor, e1, 0, e1, 2).isMoveDoable(scoreDirector)).isFalse();
        // different entity => doable
        assertThat(new SelectorBasedListChangeMove<>(variableDescriptor, e1, 0, e2, 0).isMoveDoable(scoreDirector)).isTrue();
    }
    ...
    void isMoveDoableValueRangeProviderOnEntity() {
        ...
        assertThat(new SelectorBasedListChangeMove<>(otherVariableDescriptor, entity1, 0, entity2, 0)
                .isMoveDoable(otherInnerScoreDirector))
                .isTrue();
        // different entity => invalid value
        assertThat(new SelectorBasedListChangeMove<>(otherVariableDescriptor, entity1, 1, entity2, 0)
                .isMoveDoable(otherInnerScoreDirector))
                .isFalse();
    }
    ...
    void doAndUndoMoveOnTheSameEntity(int destinationIndex, List<String> expectedValueList, int fromIndex, int toIndex) {
        // Given...
        final var sourceIndex = 2; // we're always moving V2
        var e = new TestdataListEntity("E", v0, v1, v2, v3, v4);

        // When V2 is moved to destinationIndex...
        var move = new SelectorBasedListChangeMove<>(variableDescriptor, e, sourceIndex, e, destinationIndex);
    ...
    static void assertSameProperties(Object sourceEntity, int sourceIndex, Object movedValue, Object destinationEntity,
            int destinationIndex, SelectorBasedListChangeMove<?> move) {
    ...
    void toStringTest() {
        ...
        assertThat(new SelectorBasedListChangeMove<>(variableDescriptor, e1, 1, e1, 0)).hasToString("2 {e1[1] -> e1[0]}");
        assertThat(new SelectorBasedListChangeMove<>(variableDescriptor, e1, 0, e2, 1)).hasToString("1 {e1[0] -> e2[1]}");
    }
