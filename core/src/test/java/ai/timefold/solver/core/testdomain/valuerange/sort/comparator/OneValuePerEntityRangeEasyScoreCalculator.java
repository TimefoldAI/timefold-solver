...
public class OneValuePerEntityRangeEasyScoreCalculator
        implements EasyScoreCalculator<TestdataSortableEntityProvidingSolution, HardSoftScore> {

    @Override
    public @NonNull HardSoftScore
            calculateScore(@NonNull TestdataSortableEntityProvidingSolution solution) {
        var distinct = (int) solution.getEntityList().stream()
                .map(TestdataSortableEntityProvidingEntity::getValue)
