...
class SelectorBasedListSwapMoveTest {
    ...
    void isMoveDoable() {
        ...
        assertThat(new SelectorBasedListSwapMove<>(variableDescriptor, e1, 1, e1, 1).isMoveDoable(innerScoreDirector))
                .isFalse();
        // same entity, different index => doable
        assertThat(new SelectorBasedListSwapMove<>(variableDescriptor, e1, 0, e1, 1).isMoveDoable(innerScoreDirector)).isTrue();
        // different entity => doable
        assertThat(new SelectorBasedListSwapMove<>(variableDescriptor, e1, 0, e2, 0).isMoveDoable(innerScoreDirector)).isTrue();
    }
    ...
    void isMoveDoableValueRangeProviderOnEntity() {
        ...
        assertThat(new SelectorBasedListSwapMove<>(otherVariableDescriptor, entity1, 0, entity2, 0)
    ...
    void tabuIntrospection_twoEntities() {
        ...
        moveTwoEntities.execute(innerScoreDirector);
    ...
    void tabuIntrospection_oneEntity() {
        ...
        // Do the move first because that might affect the returned values.
        moveOneEntity.execute(innerScoreDirector);
    ...
    void toStringTest() {
        ...
        assertThat(new SelectorBasedListSwapMove<>(variableDescriptor, e1, 0, e1, 1)).hasToString("1 {e1[0]} <-> 2 {e1[1]}");
        assertThat(new SelectorBasedListSwapMove<>(variableDescriptor, e1, 1, e2, 0)).hasToString("2 {e1[1]} <-> 3 {e2[0]}");
    }
