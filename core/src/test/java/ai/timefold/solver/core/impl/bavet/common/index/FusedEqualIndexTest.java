...
class FusedEqualIndexTest {
    ...
    void singleEqualBucketSharingAndOps() {
        ...
        var right1 = tuple("R1");
        var right2 = tuple("R2");
        var leftEntry = bucketF.leftDownstream().put("F", left1);
        bucketF.rightDownstream().put("F", right1);
        var right2Entry = bucketF.rightDownstream().put("F", right2);

        assertThat(bucketF.rightDownstream().size("F")).isEqualTo(2);
        ...
        bucketF.rightDownstream().remove("F", right2Entry);
        assertThat(bucketF.rightDownstream().size("F")).isEqualTo(1);
        assertThat(bucketF.isRemovable()).isFalse();

        bucketF.leftDownstream().remove("F", leftEntry);
    ...
    void emptyBucketCleanup() {
        ...
        var leftEntry = bucket.leftDownstream().put("F", tuple("L"));
        ...
        bucket.leftDownstream().remove("F", leftEntry);
    ...
    void equalPlusEqualKeyedByCompositeKey() {
        ...
        var right = tuple("R");
        bucket.rightDownstream().put(CompositeKey.of("F", 40), right);
        assertThat(bucket.rightDownstream().size(CompositeKey.of("F", 40))).isEqualTo(1);
    ...
    void equalPlusLessThanSuffixIsPerSideAndFlipped() {
        ...
        bucket.rightDownstream().put(CompositeKey.of("F", 30), rightAge30);
        bucket.rightDownstream().put(CompositeKey.of("F", 50), rightAge50);
    ...
    void hasSuffixAndIsSameBucket() {
        ...
        FusedEqualIndex<UniTuple<String>, UniTuple<String>> pureEqual = new IndexerFactory<>(equalGender()).buildFusedEqualIndex();
