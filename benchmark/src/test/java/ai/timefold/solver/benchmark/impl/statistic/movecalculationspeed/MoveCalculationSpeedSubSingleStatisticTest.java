...
public final class MoveCalculationSpeedSubSingleStatisticTest
        extends
        AbstractSubSingleStatisticTest<MoveCalculationSpeedStatisticPoint, MoveCalculationSpeedSubSingleStatistic<TestdataSolution>> {
    ...
    @Override
    protected List<MoveCalculationSpeedStatisticPoint> getInputPoints() {
        return Collections.singletonList(new MoveCalculationSpeedStatisticPoint(Long.MAX_VALUE, Long.MAX_VALUE));
    }

    @Override
    protected void runTest(SoftAssertions assertions, List<MoveCalculationSpeedStatisticPoint> outputPoints) {
        assertions.assertThat(outputPoints)
                .hasSize(1)
                .first()
                .matches(s -> s.getMoveCalculationSpeed() == Long.MAX_VALUE, "Move calculation speeds do not match.")
